...
The real cause turned out to be in Home Assistant's frontend, not the camera stream: after a tab is hidden for five minutes, Home Assistant suspends its connection and **removes the dashboard from the page to save resources**, which tears down every card — including the live video and its Picture-in-Picture window. The card now detects when it is showing a Picture-in-Picture stream and asks Home Assistant to keep the dashboard running in the background for as long as that window is open (the same mechanism as the "Keep running in background" profile option), then restores your normal setting when you close it. The floating window keeps streaming live no matter how long the tab stays in the background.

Alongside that: an ordinary tab switch now keeps the WebRTC connection alive and resumes instantly on return instead of showing a brief reconnect; the card no longer runs its own teardown-and-rebuild recovery while a Picture-in-Picture window is open (that recovery was itself able to freeze the floating window), matching Home Assistant's own WebRTC player; and stopping the stream — via the switch or by turning on privacy mode — now does an immediate, complete teardown that cannot be silently revived. Internally this replaced an earlier, more complicated recovery mechanism with a much smaller and more robust one.
