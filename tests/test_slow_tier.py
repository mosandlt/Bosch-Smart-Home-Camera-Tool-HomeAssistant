...
def _make_coord(**overrides):
    coord = SimpleNamespace(
        ...
        _slow_tier_deferred=overrides.pop("_slow_tier_deferred", set()),
        _slow_tier_defer_since=overrides.pop("_slow_tier_defer_since", {}),
    )
...
class TestDeferGate:
    ...
    def test_defers_when_stream_active_and_due(self):
        ...
        assert CAM_A in coord._slow_tier_defer_since
    ...
    def test_previously_deferred_runs_once_stream_goes_idle(self):
        coord = _make_coord(
            _slow_tier_deferred={CAM_A},
            _slow_tier_defer_since={CAM_A: time.monotonic()},
        )
        ctx = _compute_cam_context(
            coord, CAM_A, _cam_raw(), {CAM_A: {"status": "ONLINE"}}, {}, False
        )
    ...
    def test_defer_bound_forces_read_despite_active_stream(self):
        coord = _make_coord(
            ...
            _slow_tier_deferred={CAM_A},
            _slow_tier_defer_since={CAM_A: time.monotonic() - 3600.0},
        )
        ctx = _compute_cam_context(
            coord, CAM_A, _cam_raw(), {CAM_A: {"status": "ONLINE"}}, {}, True
        )
...
class TestStubCoordinatorLazyInit:
    def test_missing_deferred_attrs_lazily_initialized(self):
        ...
        lack _slow_tier_deferred/_slow_tier_defer_since entirely — the
        hasattr-based lazy-init must tolerate this without crashing."""
        coord = SimpleNamespace(_live_connections={})
        ctx = _compute_cam_context(
            coord, CAM_A, _cam_raw(), {CAM_A: {"status": "ONLINE"}}, {}, False
        )
        assert isinstance(ctx, CamContext)
        assert coord._slow_tier_deferred == set()
        assert coord._slow_tier_defer_since == {}


def _make_info_coord(**overrides):
    coord = SimpleNamespace(
        _shc_state_cache=overrides.pop("_shc_state_cache", {}),
        _privacy_set_at=overrides.pop("_privacy_set_at", {}),
        _light_set_at=overrides.pop("_light_set_at", {}),
        _notif_set_at=overrides.pop("_notif_set_at", {}),
        _WRITE_LOCK_SECS=overrides.pop("_WRITE_LOCK_SECS", 10.0),
        _live_connections=overrides.pop("_live_connections", {}),
        _lighting_switch_cache=overrides.pop("_lighting_switch_cache", {}),
        hass=MagicMock(),
        _tear_down_live_stream=MagicMock(return_value="coro"),
    )
    for k, v in overrides.items():
        setattr(coord, k, v)
    return coord
...
class TestPollCamInfoCachesPrivacy:
    def test_privacy_on_written(self):
        ...
        assert coord._shc_state_cache[CAM_A]["privacy_mode"] is True

    def test_privacy_off_written(self):
        coord = _make_info_coord()
        _poll_cam_info_caches(coord, CAM_A, {"privacyMode": "OFF"})
        assert coord._shc_state_cache[CAM_A]["privacy_mode"] is False

    def test_privacy_locked_skips_overwrite(self):
        coord = _make_info_coord(_privacy_set_at={CAM_A: time.monotonic()})
        coord._shc_state_cache[CAM_A] = {"privacy_mode": False}
        _poll_cam_info_caches(coord, CAM_A, {"privacyMode": "ON"})
        assert coord._shc_state_cache[CAM_A]["privacy_mode"] is False
    ...
    def test_external_privacy_on_transition_tears_down_active_stream(self):
        ...
        coord._tear_down_live_stream.assert_called_once_with(CAM_A)
    ...
    def test_privacy_already_on_no_teardown(self):
        ...
        coord._shc_state_cache[CAM_A] = {"privacy_mode": True}
        _poll_cam_info_caches(coord, CAM_A, {"privacyMode": "ON"})
...
class TestPollCamInfoCachesLight:
    ...
    def test_gen1_intensity_none_not_written(self):
        ...
        coord = _make_info_coord()
        coord._shc_state_cache[CAM_A] = {"front_light_intensity": 0.3}
        _poll_cam_info_caches(
            coord,
            CAM_A,
            {
        ...
        assert coord._shc_state_cache[CAM_A]["front_light_intensity"] == 0.3
    ...
    def test_gen2_no_lighting_switch_cache_yet_keeps_current_values(self):
        coord = _make_info_coord()
        coord._shc_state_cache[CAM_A] = {"camera_light": True, "front_light": True}
        _poll_cam_info_caches(
            coord,
            CAM_A,
            {
    ...
    def test_light_locked_skips_overwrite(self):
        coord = _make_info_coord(_light_set_at={CAM_A: time.monotonic()})
        coord._shc_state_cache[CAM_A] = {"camera_light": True}
        _poll_cam_info_caches(
            coord,
            CAM_A,
            {
        ...
        assert coord._shc_state_cache[CAM_A]["camera_light"] is True

    def test_light_on_none_and_cache_unset_defaults_none(self):
        coord = _make_info_coord()
        _poll_cam_info_caches(coord, CAM_A, {"featureStatus": {}})
        assert coord._shc_state_cache[CAM_A]["camera_light"] is None
...
class TestPollCamInfoCachesNotifications:
    def test_notifications_status_written(self):
        ...
        assert coord._shc_state_cache[CAM_A]["notifications_status"] == "ENABLED"

    def test_notifications_locked_skips_overwrite(self):
        coord = _make_info_coord(_notif_set_at={CAM_A: time.monotonic()})
        coord._shc_state_cache[CAM_A] = {"notifications_status": "DISABLED"}
        _poll_cam_info_caches(coord, CAM_A, {"notificationsEnabledStatus": "ENABLED"})
        assert coord._shc_state_cache[CAM_A]["notifications_status"] == "DISABLED"

    def test_empty_notifications_status_skipped(self):
        coord = _make_info_coord()
        coord._shc_state_cache[CAM_A] = {"notifications_status": "ENABLED"}
        _poll_cam_info_caches(coord, CAM_A, {})
        assert coord._shc_state_cache[CAM_A]["notifications_status"] == "ENABLED"
...
class TestPollCamInfoCachesHasLight:
    def test_has_light_written_from_feature_support(self):
        ...
        assert coord._shc_state_cache[CAM_A]["has_light"] is True

    def test_has_light_defaults_false(self):
        coord = _make_info_coord()
        _poll_cam_info_caches(coord, CAM_A, {})
        assert coord._shc_state_cache[CAM_A]["has_light"] is False
...
def _make_control_coord(**overrides):
    coord = SimpleNamespace(
        _pan_cache=overrides.pop("_pan_cache", {}),
        _lighting_switch_cache=overrides.pop("_lighting_switch_cache", {}),
    )
    for k, v in overrides.items():
        setattr(coord, k, v)
...
class TestPollCamControlPan:
    ...
    async def test_pan_fetched_when_supported_and_online(self):
        ...
        assert coord._pan_cache[CAM_A] == 42
    ...
    async def test_pan_skipped_when_no_pan_limit(self):
        ...
        assert CAM_A not in coord._pan_cache
...
class TestPollCamControlGen2Lighting:
    ...
    async def test_lighting_switch_fetched_for_gen2_online(self):
        ...
        assert coord._lighting_switch_cache[CAM_A] == {"frontLightSettings": {}}
    ...
    async def test_lighting_switch_skipped_for_gen1(self):
        ...
        assert CAM_A not in coord._lighting_switch_cache
...
def _make_slow_coord(**overrides):
    write_lock_secs = overrides.pop("_WRITE_LOCK_SECS", 10.0)
    coord = SimpleNamespace(
    ...
    def _is_write_locked(cam_id, set_at_dict):
        ts = set_at_dict.get(cam_id)
        return ts is not None and (time.monotonic() - ts) < coord._WRITE_LOCK_SECS

    coord._is_write_locked = _is_write_locked
    for k, v in overrides.items():
        setattr(coord, k, v)
    return coord
...
class TestPollSlowTierGen1AllEndpoints:
    ...
    async def test_gen1_endpoints_populate_all_caches(self):
        ...
        assert coord._wifiinfo_cache[CAM_A] == {"ssid": "home"}
        assert coord._ambient_light_cache[CAM_A] == 50
        assert data[CAM_A]["motion"] == {"sensitivity": 5}
        assert coord._firmware_cache[CAM_A] == {"upToDate": True}
        assert data[CAM_A]["recordingOptions"] == {"enabled": True}
        assert coord._unread_events_cache[CAM_A] == 3
        assert coord._commissioned_cache[CAM_A] == {"connected": True}
        assert coord._timestamp_cache[CAM_A] is True
        assert coord._notifications_cache[CAM_A] == {"movement": True}
        assert coord._rules_cache[CAM_A] == [{"id": "r1"}]
        assert coord._cloud_zones_cache[CAM_A] == [{"id": "z1"}]
        assert coord._cloud_privacy_masks_cache[CAM_A] == [{"id": "m1"}]
        ...
        assert CAM_A not in coord._privacy_sound_cache
    ...
    async def test_indoor_gen1_privacy_sound_override_included(self):
        ...
        assert coord._privacy_sound_cache[CAM_A] is False
    ...
    async def test_autofollow_and_lighting_options_gated_on_ctx(self):
        ...
        assert coord._lighting_options_cache[CAM_A] == {"opt": 1}
    ...
    async def test_autofollow_lighting_options_absent_when_not_gated(self):
        ...
        assert CAM_A not in coord._lighting_options_cache
...
class TestPollSlowTierGen2Endpoints:
    ...
    async def test_gen2_zones_and_private_areas(self):
        ...
        assert coord._gen2_zones_cache[CAM_A] == [{"id": "z1"}]
        assert coord._gen2_private_areas_cache[CAM_A] == [{"id": "p1"}]
    ...
    async def test_gen2_indoor_ii_skips_private_areas(self):
        ...
        assert CAM_A not in coord._gen2_private_areas_cache
    ...
    async def test_gen2_only_endpoints_populate_caches(self):
        ...
        assert coord._ledlights_cache[CAM_A] is True
        assert coord._lens_elevation_cache[CAM_A] == 12
        assert coord._audio_cache[CAM_A] == {"volume": 5}
        assert coord._motion_light_cache[CAM_A] == {"enabled": True}
        assert coord._ambient_lighting_cache[CAM_A] == {"enabled": False}
        assert coord._global_lighting_cache[CAM_A] == {"mode": "auto"}
        assert coord._intrusion_config_cache[CAM_A] == {"enabled": True}
        assert coord._audio_detection_cache[CAM_A] == {"detectGlassBreak": True}
    ...
    async def test_ledlights_off_state(self):
        ...
        assert coord._ledlights_cache[CAM_A] is False
    ...
    async def test_ledlights_non_dict_defaults_none(self):
        ...
        assert coord._ledlights_cache[CAM_A] is None
...
class TestPollSlowTierIndoorIIAlarm:
    ...
    async def test_alarm_settings_status_led_populate(self):
        ...
        assert coord._alarm_settings_cache[CAM_A] == {"armed": True}
        assert coord._alarm_status_cache[CAM_A]["intrusionSystem"] == "ACTIVE"
        assert coord._arming_cache[CAM_A] is True
        assert coord._icon_led_brightness_cache[CAM_A] == 3
    ...
    async def test_arming_inactive(self):
        ...
        assert coord._arming_cache[CAM_A] is False
    ...
    async def test_icon_led_brightness_clamped(self):
        ...
        assert coord._icon_led_brightness_cache[CAM_A] == 4
    ...
    async def test_icon_led_brightness_invalid_value_defaults_zero(self):
        ...
        assert coord._icon_led_brightness_cache[CAM_A] == 0
...
class TestPollSlowTierWriteLockSkips:
    ...
    async def test_motion_write_locked_not_overwritten(self):
        coord = _make_slow_coord(_motion_set_at={CAM_A: time.monotonic()})
    ...
    async def test_firmware_write_locked_not_overwritten(self):
        coord = _make_slow_coord(_firmware_set_at={CAM_A: time.monotonic()})
        coord._firmware_cache[CAM_A] = "stale"
        session = _all_endpoints_session({"firmware": {"upToDate": False}})
        await _poll_slow_tier_endpoints(
            coord, CAM_A, {}, _slow_ctx(), {CAM_A: {}}, session, HEADERS, NOOP_INTRUSION
        )
        assert coord._firmware_cache[CAM_A] == "stale"

    @pytest.mark.asyncio
    async def test_privacy_sound_write_locked_not_overwritten(self):
        coord = _make_slow_coord(_privacy_sound_set_at={CAM_A: time.monotonic()})
        coord._privacy_sound_cache[CAM_A] = True
        session = _all_endpoints_session({"privacy_sound_override": {"result": False}})
        await _poll_slow_tier_endpoints(
            coord,
            CAM_A,
        ...
        assert coord._privacy_sound_cache[CAM_A] is True

    @pytest.mark.asyncio
    async def test_timestamp_write_locked_not_overwritten(self):
        coord = _make_slow_coord(_timestamp_set_at={CAM_A: time.monotonic()})
        coord._timestamp_cache[CAM_A] = True
    ...
    async def test_ledlights_write_locked_not_overwritten(self):
        coord = _make_slow_coord(_ledlights_set_at={CAM_A: time.monotonic()})
        coord._ledlights_cache[CAM_A] = True
        session = _all_endpoints_session({"ledlights": {"state": "OFF"}})
        await _poll_slow_tier_endpoints(
            coord,
            CAM_A,
    ...
    async def test_intrusion_config_write_locked_not_overwritten(self):
        coord = _make_slow_coord(_intrusion_config_set_at={CAM_A: time.monotonic()})
        coord._intrusion_config_cache[CAM_A] = {"enabled": True}
        session = _all_endpoints_session(
    ...
    async def test_audio_detection_write_locked_not_overwritten(self):
        coord = _make_slow_coord(_audio_detection_set_at={CAM_A: time.monotonic()})
        coord._audio_detection_cache[CAM_A] = {"detectGlassBreak": True}
        session = _all_endpoints_session(
    ...
    async def test_alarm_settings_write_locked_not_overwritten(self):
        coord = _make_slow_coord(_alarm_settings_set_at={CAM_A: time.monotonic()})
        coord._alarm_settings_cache[CAM_A] = {"armed": True}
    ...
    async def test_arming_write_locked_not_overwritten_but_status_cache_still_updates(
        ...
        coord = _make_slow_coord(_arming_set_at={CAM_A: time.monotonic()})
        coord._arming_cache[CAM_A] = True
        session = _all_endpoints_session(
            {"alarmStatus": {"alarmType": "NONE", "intrusionSystem": "INACTIVE"}}
        )
        await _poll_slow_tier_endpoints(
        ...
        assert coord._arming_cache[CAM_A] is True
        assert coord._alarm_status_cache[CAM_A]["intrusionSystem"] == "INACTIVE"
...
class TestPollSlowTierNonDictFallbacks:
    ...
    async def test_unread_events_count_as_plain_int(self):
        ...
        assert coord._unread_events_cache[CAM_A] == 7
    ...
    async def test_rules_non_list_defaults_empty(self):
        ...
        assert coord._rules_cache[CAM_A] == []
    ...
    async def test_audio_non_dict_defaults_empty(self):
        ...
        assert coord._audio_cache[CAM_A] == {}
    ...
    async def test_alarm_status_non_dict_skips_arming_and_intrusion_event(self):
        ...
        assert coord._alarm_status_cache[CAM_A] == {}
        assert CAM_A not in coord._arming_cache
...
class TestPollSlowTierNon200AndExceptions:
    ...
    async def test_cancelled_error_in_one_endpoint_does_not_abort_others(self):
        ...
        assert CAM_A not in coord._wifiinfo_cache
        assert coord._ambient_light_cache[CAM_A] == 10
