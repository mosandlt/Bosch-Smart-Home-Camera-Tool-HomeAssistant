...
class TestComputeStatus:
    ...
    def test_missing_cam_id_returns_unknown(self):
        coord = _make_coord()
        assert BoschCameraCoordinator._compute_status_for(coord, "no-such-cam") == "unknown"
...
class TestCameraStatusAnnounce:
    async def test_first_observation_is_silent(self):
        coord = _make_coord()
        await BoschCameraCoordinator._async_maybe_announce_camera_status(coord, CAM_A, "online")
    ...
    async def test_online_to_offline_announces(self):
        coord = _make_coord()
        coord._last_camera_status[CAM_A] = "online"
        await BoschCameraCoordinator._async_maybe_announce_camera_status(coord, CAM_A, "offline")
    ...
    async def test_unknown_transitions_are_silent(self):
        ...
        coord = _make_coord()
        coord._last_camera_status[CAM_A] = "online"
        await BoschCameraCoordinator._async_maybe_announce_camera_status(coord, CAM_A, "unknown")
        coord.hass.services.async_call.assert_not_called()
    ...
    async def test_per_camera_state_is_isolated(self):
        ...
        await BoschCameraCoordinator._async_maybe_announce_camera_status(coord, CAM_A, "offline")
        await BoschCameraCoordinator._async_maybe_announce_camera_status(coord, CAM_B, "offline")
        assert coord.hass.services.async_call.await_count == 2
        titles = {c.args[2]["title"] for c in coord.hass.services.async_call.await_args_list}
