...
| Version | Changes |
|---------|---------|
| **v9.1.4** | **In-place reauth button.** When the refresh token is permanently invalidated on Keycloak's side (`invalid_grant`), the integration now raises `ConfigEntryAuthFailed` instead of getting stuck in a retry loop. Home Assistant automatically shows a **Reconfigure** button on the integration card, which runs the full auto-login flow (browser → Bosch SingleKey ID → automatic redirect) and **updates the existing entry in place** — options, entities, automations, FCM config, and SMB settings are all preserved. No more remove-and-readd. German + English translations for the reauth dialog. Fixes the recovery path for [#5](https://github.com/mosandlt/Bosch-Smart-Home-Camera-Tool-HomeAssistant/issues/5). |
