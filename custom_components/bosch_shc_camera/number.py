...
class BoschSpeakerLevelNumber(_BoschEntityBase, NumberEntity):  # type: ignore[misc]
    ...
    async def async_set_native_value(self, value: float) -> None:
        ...
        and BoschIntercomSwitch (same /audio endpoint, same audio_cache) so a
...
class BoschIntrusionSensitivityNumber(_BoschGen2NumberBase):
    ...
    Write-lock timestamp intrusion_config_set_at is set after successful PUT to
...
class BoschIntrusionDistanceNumber(_BoschGen2NumberBase):
    ...
    Write-lock timestamp intrusion_config_set_at is set after successful PUT (same guard
