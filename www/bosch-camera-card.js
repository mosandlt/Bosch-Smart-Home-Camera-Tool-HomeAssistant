...
class BoschCameraCard extends HTMLElement {
  ...
  _render() {
    ...
    this.shadowRoot.getElementById("btn-light").addEventListener("click", () => this._toggleSwitch(this._entities.light));
    this.shadowRoot.getElementById("btn-privacy").addEventListener("click", () => this._toggleSwitch(this._entities.privacy));
  ...
  async _startWebRTC(video, activateVideo) {
    ...
    await new Promise((resolve, reject) => {
      const timeout = setTimeout(() => reject(new Error("WebRTC: no track within 10s")), 1e4);
      pc.ontrack = ev => {
  ...
  _update() {
    ...
    const streamBadgeState = this._startingLiveVideo ? "connecting" : isStreaming ? "streaming" : "idle";
