...
| Version | Changes |
|---------|---------|
| **v10.3.13** | **Card v2.10.3 — `low_bandwidth: true` option: HLS never auto-starts, user taps ▶ to watch.** New YAML config option `low_bandwidth: true` for the Lovelace card. When enabled, opening the dashboard no longer automatically starts the HLS video stream — only the snapshot image is loaded (a few KB vs. 7.5 Mbps continuous). A centered ▶ overlay ("Live anzeigen / Tippen zum Starten") appears over the snapshot whenever the stream is active but HLS has not been unlocked yet. Tapping the overlay (or the Live Stream button) starts HLS exactly as before. Once the user stops the stream, the lock resets so the next stream-on shows the overlay again. Works with both manual stream starts (Live Stream button) and automation-triggered stream-on. Default `low_bandwidth: false` — existing cards unchanged. Usage: add `low_bandwidth: true` to the card YAML. |
