...
| Version | Changes |
|---------|---------|
| **v10.4.8** | **Local RCP+ READ via the ad-hoc `cbs-…`-user from `PUT /connection`.** New module `local_rcp.py` issues HTTP Digest reads against `https://<cam>:443/rcp.xml` (LOCAL session) and HTTP Basic-empty against `https://proxy-XX:42090/{hash}/rcp.xml` (REMOTE session — Cloud-Proxy fallback when HA is not on the LAN). Verified locally on Gen2 Outdoor FW 9.40.25: 10 reads/10 s did not rotate creds or kill the running stream — only `PUT /connection` rotates, normal RCP reads are safe. **Two fields are pulled opportunistically after each successful stream start (`_try_live_connection_inner` post-warmup) and exposed as camera entity diagnostic attributes:** `rcp_privacy_mode` (from `0x0d00` P_OCTET, byte[1]==1 means ON) and `rcp_led_dimmer` (from `0x0c22` T_WORD, 0–100 %). Plus `rcp_state_age` (seconds since last successful read) and `rcp_state_source` (`local` / `remote`). The values *refine* `_shc_state_cache` only when SHC is unconfigured / unreachable (entry is `None`) — SHC remains the primary truth-source when present. Useful as cross-check (a SHC↔RCP mismatch surfaces stale-cache issues immediately) and as a fallback for users who don't have an SHC. Read-only — writes still need the `service`-account credentials Bosch will release with the Sommer 2026 local-user feature. **Note:** the local `/rcp.xml` endpoint returns XML (not the binary TLV format the Cloud-Proxy uses on the same path), so the parser is XML-based; if a future REMOTE-only test surfaces binary, extend `_parse_rcp_xml` with a binary fallback. |
