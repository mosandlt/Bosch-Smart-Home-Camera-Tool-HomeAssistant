...
async def promote_to_local(coordinator: BoschCameraCoordinator, cam_id: str) -> None:
    ...
    — with `stream_fell_back` already cleared by the caller — runs the
...
async def remote_session_terminator(
    ...
    Generation-tracked the same way as `auto_renew_local_session`: any
    ...
    try:
        ...
        # this terminator is itself registered in `renewal_tasks[cam_id]`
        # (`replace_renewal_task`), and `tear_down_live_stream`'s first
