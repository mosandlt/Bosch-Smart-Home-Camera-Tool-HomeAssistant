...
class TestAsyncCameraImageWrapper:
    ...
    async def test_returns_impl_result_when_present(self):
        ...
        out = await BoschSHCCamera.async_camera_image(cam)
    ...
    async def test_returns_placeholder_when_impl_returns_none(self):
        ...
        out = await BoschSHCCamera.async_camera_image(cam)
        assert out == BoschSHCCamera._PLACEHOLDER_JPEG
    ...
    async def test_cancelled_error_propagates(self):
        ...
        with pytest.raises(asyncio.CancelledError):
            await BoschSHCCamera.async_camera_image(cam)
...
class TestAsyncCameraImageImplLocalDigest:
    ...
    async def test_local_digest_success_caches_image(self):
        ...
        with patch(
            ...
            out = await BoschSHCCamera._async_camera_image_impl(cam)
...
class TestYuv422EdgeCases:
    ...
    def test_zero_sized_input_returns_none(self):
        from custom_components.bosch_shc_camera.camera import BoschSHCCamera
        cam = _make_camera()
        out = BoschSHCCamera._yuv422_to_jpeg(cam, b"")
        assert out is None
