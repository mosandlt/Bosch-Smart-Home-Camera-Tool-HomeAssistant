...
**Weekly promotion (beta → stable).** Every **Friday at 18:00 (Europe/Berlin)** the newest open beta is automatically promoted to a stable release, provided its required CI checks are green — see [`promote-beta.yml`](.github/workflows/promote-beta.yml). The resulting stable release is a collection of everything shipped across that version's beta iterations that week, not just the single newest fix. If a week has no open beta, there's no stable release that week either. If you're testing a beta and want your feedback to count before it ships stable, report back on the linked GitHub issue before Friday. (Operational note: unlike a beta, a stable release's CHANGELOG.md section is mandatory — if one goes to promote without a matching `## [vX.Y.Z]` entry already written, the promotion tags `main` but the actual GitHub Release publish step fails until the entry is added and the workflow is re-run.)
