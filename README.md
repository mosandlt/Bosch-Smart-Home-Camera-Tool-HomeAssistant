...
**2026-08-16 correction**: the "plays ~8-15 s → stalls → rebuilds, repeating" symptom below was originally root-caused to an upstream cloudflared limitation. That diagnosis was wrong. The real cause was a card-side bug, fixed in **v16.1.9**: the live-view's freeze watchdog only kept renewing its "still alive" heartbeat for WebRTC playback, not for HLS — so any pure-HLS remote session (Cloudflare Tunnel, no VPN, is a common way to end up on HLS) looked permanently frozen ~10 s after its first frame and triggered a full reconnect, forever, even though HLS itself was decoding fine the whole time. Reporter-confirmed fixed on v16.1.9. Kept here for anyone who hit the same symptom on an older version: update to v16.1.9+. See [issue #67](https://github.com/mosandlt/Bosch-Smart-Home-Camera-Tool-HomeAssistant/issues/67) for the corrected write-up.

<details>
<summary>Original (incorrect) 2026-08-09 investigation — kept for history</summary>

On one real-world setup, a remote (Cloudflare-Tunnel, no VPN) live stream kept flapping (WebRTC failing fast → HLS → plays ~8-15 s → stalls → rebuilds, repeating) even with the unbuffer patch confirmed active and correctly configured (verified via the `curl` check above and the add-on's generated ingress config). At the time this was root-caused to an open, unresolved upstream cloudflared limitation ([cloudflare/cloudflared#1449](https://github.com/cloudflare/cloudflared/issues/1449)): GET requests can buffer completely through cloudflared regardless of header/content-type tricks, while POST requests stream fine — HLS delivery is all-GET, and streaming the identical camera over a VPN instead of the tunnel worked with no issues. That correlation was real, but the actual cause turned out to be the card-side bug described above, not cloudflared itself — the VPN path happens to avoid the same HLS fallback pattern this bug required to trigger.

</details>

If a genuine buffering issue against cloudflared ever resurfaces after updating to v16.1.9+, the upstream [cloudflare/cloudflared#1449](https://github.com/cloudflare/cloudflared/issues/1449) report is still worth checking, and `cf_unbuffer.py` (above) remains active as a mitigation either way.
