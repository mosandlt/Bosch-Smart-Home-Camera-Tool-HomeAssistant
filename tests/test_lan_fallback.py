...
def _make_coord(**overrides) -> SimpleNamespace:
    ...
    coord.data = {CAM_A: {"info": {"title": "Terrasse"}}, CAM_B: {"info": {"title": "Innenbereich"}}}
    for k, v in overrides.items():
        setattr(coord, k, v)
    # Bind the in-grace helper so `is_lan_reachable` (which calls self._in_local_write_grace)
    # works against the stub.
    coord._in_local_write_grace = BoschCameraCoordinator._in_local_write_grace.__get__(coord)
...
class TestOutagePingAll:
    ...
    async def test_runs_again_after_throttle_window(self):
        ...
        with patch("custom_components.bosch_shc_camera.time.monotonic", return_value=1000.0):
            await BoschCameraCoordinator._async_outage_ping_all(coord)
        with patch("custom_components.bosch_shc_camera.time.monotonic", return_value=1100.0):
            await BoschCameraCoordinator._async_outage_ping_all(coord)
