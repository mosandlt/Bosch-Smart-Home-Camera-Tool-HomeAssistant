...
- **FCM device-token backend-load follow-up (Bosch camera backend team, Michael Kihl HOME/EBT-S, 2026-09-04): despite v16.1.10/v16.1.11, some HA instances still register >30 new FCM device tokens/day at Bosch's backend.** Root cause found in `_async_run_fcm_supervisor()`: the benign "soft-restart-threshold" hard-heal trigger (3 consecutive listener restarts with no push received) purges FCM credentials and mints a brand-new token on every occurrence, and was deliberately excluded from the existing CONFIRMED-problem backoff ladder — on a persistently flaky WAN/router this benign path can recur every 1.5-5 min indefinitely, each time purging good credentials and re-registering with Bosch. Fixed with a SEPARATE, lighter backoff (`soft_trigger_heal_streak`, same `FCM_SUPERVISOR_BACKOFF_SEC` ladder) specifically for this trigger — reset by either `FCM_SUPERVISOR_SUSTAINED_UPTIME_SEC` (600s) of continuous listener uptime OR a received push (whichever comes first), so a genuinely-recovering connection is never penalized, while a truly flaky one now backs off exactly like the CONFIRMED-problem path already does. Also wired in Bosch's newly-offered `PUT v11/registration/logout?deviceToken=<token>` endpoint (new `deregister_fcm_with_bosch()`) — called best-effort, right before every hard-heal purge discards the currently-registered token, so Bosch's backend no longer accumulates an abandoned registration per purge. This endpoint is not in Bosch's public API docs (offered informally via email) — SELF_VERIFY: its exact scope (per-device-token vs. account-wide) is assumed from Bosch's own description, not yet live-verified; see `docs/api-reference.md` § FCM.
