...
// key falls back to English, so partial translations are safe. All visible card
// + editor chrome routes through cardT()/this._t() — no hard-coded UI language.
const CARD_I18N = {
  en: {
    // ── runtime chrome ──
    ...
    // ── issue #45 follow-up: more hardcoded German with no _t() key at all ──
...
// Module-level translator so every custom element (cards AND their separate
// editor elements) shares one string table. cardLang() maps hass.language to a
// table key; cardT() resolves a key with English fallback. The set of languages
// mirrors the integration's own translations/ folder.
const CARD_LANGS = ["de", "en", "es", "fr", "it", "nl", "pl", "pt", "ru", "uk", "zh-Hans"];
function cardLang(hass) {
  const l = ((hass && hass.language) || "en").toLowerCase();
  if (l.startsWith("zh")) return "zh-Hans"; // any Chinese variant → Simplified
  const pre = l.slice(0, 2); // strip region suffix: pt-BR → pt, en-GB → en
...
class BoschCameraCard extends HTMLElement {
  constructor() {
    ...
    this._liveStreamStalled = false; // true when a live <video> is frozen (no fresh frames) — badge shows "connecting", not "Live". 2026-06-22
    this._hiddenTeardownTimer = null; // grace timer that tears down a hidden, non-PiP stream (community pattern). 2026-06-22
    ...
    this._prevFramesDecoded   = null;  // last getStats() inbound-rtp video framesDecoded — decoder-level freeze oracle. 2026-06-22
    this._framesDecodedSeenAt = 0;     // performance.now() of the last framesDecoded ADVANCE (0 = no baseline yet)
    this._statsCheckInFlight  = false; // single-flight guard for the async getStats freeze check
    ...
    this._ZOOM_MAX = 4;
    this._zoom = { scale: 1, tx: 0, ty: 0, prevDist: -1, lastTap: 0 };
  ...
  connectedCallback() {
    ...
    this._onFullscreenChange = () => {
      ...
      if (!this._isNativeFullscreen()) { _boschFsExitAt = Date.now(); this._resetZoom(); }
  ...
  // Wire the apple-style audio pill. Desktop (hover): tap = mute/unmute, hover =
  // volume slider. Mobile/touch + iOS: just tap = mute/unmute — NO slider. On
  // touch the slider was fiddly (and on iOS volume is read-only anyway), so the
  // pill is a clean on/off; volume lives on the number.<cam>_audio_volume entity
  // for anyone who wants it (slider card / automation). 2026-06-01.
  _wireAudioPill() {
    const noSlider = this._isIOS() || !this._hasHover();
    this.classList.toggle("audio-no-volume", noSlider);
    // Card option to hide the whole audio control.
    this.classList.toggle("audio-hidden", !!(this._config && this._config.show_audio === false));
    // Drop any stale touch-dismiss listener from an earlier render/build.
  ...
  _setVideoVolume(v) {
    ...
    if (this._useCardAudio()) this._cardSaveVolume(v);
    else this._setBackendVolume(v);
    this._refreshAudioPill();
  }

  // Audio source. By default the backend entities (switch.<cam>_audio +
  // number.<cam>_audio_volume) are the synced, automatable source of truth.
  // With use_card_audio_settings:true the card DECOUPLES from them and keeps
  // mute + volume per-browser (localStorage) — for users who don't want the
  // global/shared behaviour. 2026-06-01.
  _useCardAudio() {
  ...
  //    it as a 0-1 fraction; _setBackendVolume() writes it back so HA pushes the
  //    change to every open card. (mute/on-off is owned by switch.<cam>_audio.) ──
  _entityVolume() {
    const n = parseFloat(this._hass?.states[this._entities.audioVolume]?.state);
    return n >= 0 && n <= 100 ? n / 100 : 0.5;
  }
  _setBackendVolume(v01) {
    if (!this._hass || !this._entities.audioVolume) return;
    const pct = Math.round(Math.max(0, Math.min(1, v01)) * 100);
    // Skip a redundant write if the entity already holds this value (avoids a
    // feedback loop with the live-sync re-apply in _update()).
  ...
  set hass(hass) {
    ...
    if (firstHass) {
      ...
      // BUT never stack the refresh overlay on top of the privacy gate: when
      // privacy is ON the placeholder owns the whole tile, so a cached image +
      // "Aktualisiere…" would double-overlay (lock + spinner, #15/privacy
      // report). _update() already showed the gate above; suppress loading here.
      const _privacyNow =
  ...
  // Localize the tap-to-play overlay's static template text (issue #45
  // follow-up scan, realKim-dotcom 2026-07-13): the label/hint were hardcoded
  // German in the shell template and never routed through _t() at either of
  // the two call sites that reveal this overlay (iOS bfcache-resume NotAllowedError,
  // Android WebView autoplay-blocked NotAllowedError). Called right before each
  // reveal so a language change is always reflected.
  ...
  _showPlayGate() {
    ...
    // Gate owns the UX now — stop any in-flight connect poll + keepalive timers
    // so their later ticks can't repaint the loading spinner ON TOP of the gate
    // (the "Kamera wird aufgeweckt…" / "LAN-Stream — ca. 25–35 s" overlay bled
    // through behind the gate on slow-warming cameras otherwise). The user's tap
  ...
  _onVisibilityChange() {
    if (document.visibilityState === "hidden") {
      // Nobody is watching a plain hidden tab. The go2rtc signaling WebSocket
      // times out in the background (AlexxIT/WebRTC#121) and the <video> can
      // freeze to a still that paused===false hides from the resume path — the
      // exact "showed Live but was a frozen frame, only a browser reload fixed
      // it" report. Tear the local stream down after a short grace and rebuild
      // it fresh on return, matching HA core's ha-web-rtc-player and AlexxIT/
      // go2rtc (all teardown-on-hidden, rebuild-on-visible). PiP is exempt — a
      // floating PiP IS being watched and keeps its own recovery. 2026-06-22.
      this._scheduleHiddenTeardown();
      this._startRefreshTimer();
      return;
    }
    if (document.visibilityState === "visible") {
      // Returned before the grace elapsed → keep the stream, cancel teardown.
      if (this._hiddenTeardownTimer) {
        clearTimeout(this._hiddenTeardownTimer);
        this._hiddenTeardownTimer = null;
      }
  ...
  _resumeLiveStreamIfNeeded() {
    ...
    // Not paused, nominally live — but a background WS i/o timeout
    // (AlexxIT/WebRTC#121) can leave the element on a FROZEN still with
    // paused===false. The old code returned here and the stream stayed a zombie
    // "Live" still until a full browser reload. The teardown-on-hidden path
    // covers the common case, but PiP and sub-grace tab-flips skip it — so
    // verify a FRESH frame is actually presented after return and force a
    // PiP-safe reconnect if not. rVFC is suppressed for a hidden non-PiP
    // element, so _boschLastFrameAt is always stale right after return; give the
    // stream ~3s, then require either a new presented frame OR currentTime
    // advancing since we became visible. Runs only while VISIBLE, so the JS
    // thread is awake (no iOS thread-suspend false positive). 2026-06-22.
    if (video) {
      const t0  = performance.now();
      const ct0 = video.currentTime;
      setTimeout(() => {
        if (!this.isConnected || document.visibilityState !== "visible") return;
        if (!this._liveVideoActive || this._reconnectingLiveVideo || this._stoppingLiveVideo) return;
        const v = this.shadowRoot?.getElementById("cam-video");
        if (!v || v.paused) return; // a fresh pause is handled by the pause-guard / paused branch
        // rVFC (frames actually composited) is the reliable liveness proof for
        // BOTH WebRTC and HLS. currentTime is only trustworthy for HLS/MSE — for
        // a WebRTC MediaStream Chrome backs it with the AUDIO clock, so it can
        // tick on a frozen video OR stall on a healthy one (see the stall-checker
        // note ~line 6113). So trust currentTime only on the HLS path, or as a
        // last resort when rVFC is unsupported. 2026-06-22 (verify-agent finding).
        const rvfcSupported = typeof v.requestVideoFrameCallback === "function";
        const freshFrame    = v._boschLastFrameAt != null && v._boschLastFrameAt > t0;
        const timeAdvanced  = v.currentTime > ct0 + 0.05;
        // currentTime is a reliable liveness proof ONLY for HLS/MSE. For a WebRTC
        // MediaStream Chrome backs currentTime with the AUDIO clock, so it keeps
        // ticking on a FROZEN video — on iOS (no rVFC fast-escalate) trusting it let
        // a frozen WebRTC PiP pass this check forever. So require a real presented
        // frame (rVFC) for WebRTC on iOS; trust currentTime on the HLS path, OR on a
        // NON-iOS browser that has WebRTC but NO rVFC (Firefox <116) — there the
        // strict rule would needlessly reconnect a healthy stream on every tab-return.
        // B5 fix 2026-06-22 (+ verify-agent Firefox<116 regression guard).
        const liveProof = freshFrame
          || (timeAdvanced && (!!this._hls || (!rvfcSupported && !this._isIOS())));
        if (!liveProof) {
          this._scheduleLiveRecovery("no fresh frame within 3s of tab-return");
        }
      }, 3000);
    }
  }

  // Tear down a hidden, non-PiP live stream after a short grace so it can't sit
  // frozen in the background. Rebuilt fresh on tab-return via
  // _resumeLiveStreamIfNeeded (the !_liveVideoActive branch → _startLiveVideo).
  // Matches HA core's ha-web-rtc-player + AlexxIT/go2rtc (teardown-on-hidden).
  // Grace tolerates quick alt-tab; PiP is exempt (it's being watched). The
  // backend stream switch is left ON, so only this client's frozen transport is
  // dropped — the next visible start negotiates a clean WebRTC/HLS session.
  // 2026-06-22.
  _scheduleHiddenTeardown() {
    if (this._hiddenTeardownTimer) return;        // already scheduled
    if (!this._liveVideoActive) return;           // nothing live to tear down
    this._hiddenTeardownTimer = setTimeout(() => {
      this._hiddenTeardownTimer = null;
      if (document.visibilityState !== "hidden") return; // came back during grace
      if (!this.isConnected || !this._liveVideoActive) return;
      if (this._reconnectingLiveVideo || this._stoppingLiveVideo) return;
      const video = this.shadowRoot?.getElementById("cam-video");
      if (this._ownsNativePresentation(video)) return; // PiP/native-fullscreen IS being watched — keep it
      this._stopLiveVideo();
    }, BACKGROUND_TEARDOWN_GRACE_MS);
  }

  // Single setter for the frozen-stream flag so the badge ("Live" vs "Verbinde")
  // tracks REAL frame liveness, not just "a peer connection was once built". A
  // frozen still used to keep showing "Live" until a browser reload. Re-renders
  // only when visible (the badge isn't on screen while hidden). 2026-06-22.
  _setLiveStalled(stalled) {
    if (this._liveStreamStalled === stalled) return;
    this._liveStreamStalled = stalled;
    if (this.isConnected && document.visibilityState === "visible") this._update();
  }
  ...
  _render() {
    ...
    // Fullscreen digital zoom: attach pinch/pan/double-tap/wheel handlers to the
    // video wrapper (re-attached each render; handlers no-op unless in fullscreen)
    // and re-apply the current zoom transform to the freshly-rendered video/img.
  ...
  async _startLiveVideo(attempt = 1) {
    ...
    const activateVideo = () => {
      ...
      this._liveStreamStalled  = false; // fresh start → not stalled (badge "Live"). 2026-06-22
      this._startingLiveVideo  = false;
      ...
      // once:true — if activateVideo runs twice (e.g. two WebRTC ontrack fires)
      ...
      this._stallChecker = this._armInterval(() => {
        ...
        // Decouple the "Live" badge from "a peer connection exists": the moment a
        // presented-frame freeze is confirmed (>10s), drop the badge to
        // "connecting" so the user never sees "Live" over a frozen still. Cleared
        // by the rVFC onFrame the instant fresh frames flow again. 2026-06-22.
        if (frameFrozen) this._setLiveStalled(true);
        ...
        lastTime = video.currentTime;
        // Decoder-level freeze oracle (getStats framesDecoded) — an independent,
        // compositor-agnostic signal that also catches Chrome 145+ muted-background
        // pause and go2rtc silent stalls that emit no track/connection event and
        // keep currentTime ticking. iOS excepted (a suspended JS thread stalls the
        // poll itself → false positive on thaw; iOS is covered by the event paths).
        // _scheduleLiveRecovery is idempotent so this can't double-fire with rVFC.
        // 2026-06-22.
        if (!this._isIOS()) {
          this._checkWebrtcFreeze().then((statsFrozen) => {
            if (statsFrozen && this._liveVideoActive) {
              this._setLiveStalled(true);
              this._scheduleLiveRecovery("webrtc framesDecoded frozen >10s");
            }
          });
        }
      }, 5000);
    ...
    try {
      ...
      const startPlay = () => {
        ...
        video.play()
          ...
          .catch((err) => {
            ...
            // Any other error: retry play once; if it STILL fails the element is
            // wedged (decode error / detached source) — escalate to a reconnect
            // instead of leaving _liveVideoActive=true on a frozen element that
            // never recovers and never increments _streamDropCount. B7 fix 2026-06-22.
            ...
            setTimeout(() => {
              ...
              if (!v) return;
              v.muted = true;
              Promise.resolve(v.play()).catch((e2) => {
                // NotAllowedError = autoplay/visibility policy refusal, not a dead
                // stream — don't reconnect (the pause-guard / next gesture handles
                // it). Only a genuine play failure escalates. 2026-06-22 (verify-agent).
                if (e2 && e2.name === "NotAllowedError") return;
                console.warn("bosch-camera-card: muted play retry failed, reconnecting:", e2?.message);
                if (this._reconnectingLiveVideo || this._stoppingLiveVideo) return;
                this._reconnectingLiveVideo = true;
                this._stopLiveVideo();
      ...
      if (Hls && Hls.isSupported()) {
        ...
        this._stallCount = 0;
        hls.on(Hls.Events.FRAG_LOADED, () => { this._stallCount = 0; this._hlsNetworkErrorCount = 0; });
        ...
        hls.on(Hls.Events.ERROR, (_ev, data) => {
          ...
          if (data.type === Hls.ErrorTypes.NETWORK_ERROR) {
            // hls.startLoad() retries the SAME url. If the backend url is dead
            // (rotated Bosch creds / dead proxy port) this loops forever and the
            // stream sits silently frozen — it never escalates to a backend rewarm.
            // Retry a few times, then fall through to a full reconnect so
            // _maybeForceBackendRewarm() can re-issue fresh creds. B3 fix 2026-06-22.
            this._hlsNetworkErrorCount = (this._hlsNetworkErrorCount || 0) + 1;
            if (this._hlsNetworkErrorCount <= 3) {
              hls.startLoad();
            } else {
              this._hlsNetworkErrorCount = 0;
              console.warn("bosch-camera-card: hls.js fatal NETWORK_ERROR persists, reconnecting", data);
              this._reconnectingLiveVideo = true;
              this._stopLiveVideo();
              if (this._isStreaming && this._isStreaming()) {
                setTimeout(() => { if (!this.isConnected) return; this._reconnectingLiveVideo = false; this._reconnectAfterStreamDrop(); }, 2000);
    ...
    pc.ontrack = (ev) => {
      ...
      if (ev.track.kind === "video") {
        ...
        ev.track.onmute = () => {
          ...
          // Without this guard the old closure arms _trackMuteTimer and, 6s later,
          // calls _scheduleLiveRecovery on the HEALTHY NEW stream → reconnect loop
          ...
          this._trackMuteTimer = this._armTimer(() => {
            ...
            // down the healthy new stream. B1 fix 2026-06-22.
            if (this._webrtcPc !== pc) return;
  ...
  _reconnectAfterStreamDrop() {
    ...
    if (this._streamDropCount >= 2) {
      // The go2rtc source is dead — only a backend rewarm helps. If a rewarm fires,
      // reset and wait for it. If one is already cooling down (<20s ago), DON'T fall
      // through to _startLiveVideo() and hammer the dead source — the in-flight
      // rewarm brings the camera back via the _update() auto-start gate.
      // 2026-06-22 (verify-agent: cooldown-hammer fix).
      if (this._maybeForceBackendRewarm()) this._streamDropCount = 0;
      return;
    }
    const cam = this._hass?.states[this._entities.camera];
    if (cam?.state === "streaming") {
  ...
  _liveStallTickFromWorker() {
    ...
    // Un-throttled background getStats freeze check (worker wakeup → main-thread
    // poll). Catches a decoder-level freeze the rVFC signal misses — e.g. Chrome
    // 145+ pausing the muted background <video> stops decoding without a track
    // event. iOS excepted (same thread-suspend caveat as above). 2026-06-22.
    if (!this._isIOS()) {
      this._checkWebrtcFreeze().then((statsFrozen) => {
        if (statsFrozen && this._liveVideoActive) {
          this._scheduleLiveRecovery("webrtc framesDecoded frozen >10s (bg worker)");
        }
      });
    }
  }
  ...
  // Decoder-level freeze oracle via RTCPeerConnection.getStats(). The most
  // reliable cross-browser "is the WebRTC video actually frozen" signal: when
  // `bytesReceived` keeps climbing but `framesDecoded` stops advancing, the picture
  // is frozen at the DECODER. This is independent of the compositor (so it catches
  // a hidden-tab / background freeze that rVFC — which piggybacks on the throttled
  // compositor — can miss) and independent of the cause: it fires the same whether
  // go2rtc silently stalled (RTP flowing, no new frames, connectionState still
    ...
    try {
      ...
      stats.forEach((r) => {
        if (r.type === "inbound-rtp" && (r.kind === "video" || r.mediaType === "video")
          ...
          // Take the HIGHEST framesDecoded across reports: an RTX/retransmission
          // inbound-rtp can also be kind:"video" with a permanently-0 framesDecoded,
          // and if it iterated LAST it would peg the oracle at 0 → false freeze on a
          // healthy stream ~10s in. 2026-06-22 (verify-agent finding).
          framesDecoded = r.framesDecoded;
        }
      });
      if (framesDecoded == null) return false;              // no video inbound-rtp report yet
      const now = performance.now();
      if (this._prevFramesDecoded == null || framesDecoded > this._prevFramesDecoded) {
        ...
        return false;                                       // frames are advancing → healthy
      }
      // framesDecoded flat since the last poll — frozen if it has not advanced >10s.
      return this._framesDecodedSeenAt > 0 && (now - this._framesDecodedSeenAt) > 10000;
    } catch {
      return false;                                         // getStats unavailable/raced → no verdict
    } finally {
  ...
  _scheduleLiveRecovery(reason) {
    ...
    // a native presentation surface (PiP window OR iOS native <video> fullscreen):
    //  (1) Hidden + none of the above: nobody is watching the in-page tile. A
    //      background track-mute / connectionState blip would otherwise thrash a
    //      stream that survives the backgrounding and resumes fine on return
    //      (extended teardown grace); the visibilitychange→visible path
    //      recovers it then.
    //  (2) PiP-owned OR iOS-native-fullscreen-owned: our recovery does
    //      srcObject=null + video.load(), which tears the same WebKit/Chrome
    //      compositor surface link PiP depends on (crbug 894317) and freezes the
    //      floating window — or, for iOS native fullscreen, can leave a LATER PiP
    //      request on the same <video> stuck spinning forever even after the user
    //      already left fullscreen (2026-07-13 iOS PiP-after-fullscreen bug). HA
    //      core ha-web-rtc-player does the same for PiP — it never recovers while
    //      PiP is open. Recovery resumes on leavepictureinpicture / presentation-
    //      mode-change-to-inline / tab-return.
    //  `_ownsNativePresentation()` is a live read (browser-authoritative
    //  pictureInPictureElement + the iOS-webkit `_boschPipActive` mirror + iOS
    //  `webkitPresentationMode`), so no extra state to manage. 2026-06-24,
    //  extended 2026-07-13.
    {
      const v = this.shadowRoot && this.shadowRoot.getElementById("cam-video");
      if (this._ownsNativePresentation(v)) return;
    ...
    if (this.isConnected && this._isStreaming && this._isStreaming()) {
      // 1000ms (was 2000ms): _stopLiveVideo set video.srcObject=null, so a PiP
      // window stays OPEN but BLANK during this gap — halve it. NOT 500ms: although
      // the JS pc is closed synchronously, go2rtc's server-side ICE/DTLS teardown is
  ...
  _stopLiveVideo() {
    ...
    this._liveVideoActive   = false;
    this._liveStreamStalled = false; // no live element → no stalled state. 2026-06-22
    // Reset the getStats freeze oracle so the next stream starts from a clean
    // baseline — a stale framesDecoded value must not look "frozen" instantly. 2026-06-22.
  ...
  _update() {
    ...
    // Privacy ON → the placeholder owns the tile; never show a "Verbinde"/Live
    // badge over it (legacy non-apple mode has no CSS suppression for this). Own
    // local — the function-scope `privacyOn` is declared further down (TDZ here).
    const badgePrivacyOn = this._optimistic[ents.privacy] !== undefined
      ? this._optimistic[ents.privacy] === "on"
      : this._hass?.states[ents.privacy]?.state === "on";
    // "streaming" requires REAL frame liveness, not just _liveVideoActive — a
    // frozen still (no fresh frames, _liveStreamStalled) falls through to
    // "connecting" instead of lying "Live". 2026-06-22.
    const streamBadgeState = isOffline ? "offline"
                           : (badgePrivacyOn ? "idle"
                           : ((this._liveVideoActive && !this._liveStreamStalled) ? "streaming"
    ...
    if (apBadge) {
      ...
      if (streamBadgeState === "offline") {
        ...
        // "Live"/"Offline" are kept as universal loanwords like the rest of this
        // badge/pill chrome, but "Verbinde" was a bare German verb form with no
        // _t() key — the one badge state that actually read as broken/foreign
        // text to a non-German user. (issue #45 follow-up)
        apBadge.className = "ap-badge connecting"; apBadge.textContent = this._t("badge_connecting");
      } else {
    ...
    if (this._playGateActive) {
      this._setLoadingOverlay(false);
      // Belt-and-suspenders: ensure no stream-ready poll keeps ticking behind the
      // gate (the early return below would otherwise skip the off-path cleanup),
      // which could call _startLiveVideo() and bypass the gate. _showPlayGate
      // also clears this, but re-assert here so it can never start hidden.
      this._waitingForStream = false;
      return;
    }
    ...
    // issue #45 follow-up scan (realKim-dotcom 2026-07-13): "Letztes: " prefix
    // was hardcoded German regardless of hass.language. Only reachable in
    // legacy (non-apple_style) layout — apple-style hides .img-overlay — but
    // still a real gap for anyone with apple_style:false.
    ...
    if (apLastEvent && apLastEventText) {
      ...
      if (hasEvent && lastEventState?.state) {
        try {
          ...
          if (!isNaN(d)) {
            ...
            // issue #45 follow-up scan (realKim-dotcom 2026-07-13): was always
            // de-DE regardless of hass.language.
            const loc = cardLang(this._hass);
            pretty = sameDay
      ...
      // Also hide while the tap-to-play gate is up — the badge (z4) otherwise
      // bleeds through the gate's translucent backdrop behind the play button.
    ...
    // and the controls felt stuck (RkcCorian, #22). Done BEFORE hiding the
    // spinner so the in-flight connect flags (_streamConnecting/_waitingForStream)
    // are already cleared — otherwise the _setLoadingOverlay(false) suppression
    // guard blocked the hide and the spinner stuck on top of the placeholder.
    // Covers a connect in flight, not just an already-live video.
    if (this._lastPrivacy !== true && privacyOn &&
        (this._liveVideoActive || this._streamConnecting || this._waitingForStream)) {
      this._stopLiveVideo();
    }
    ...
    // (Privacy-ON teardown is handled earlier, before the spinner is hidden, and
    // already covers _liveVideoActive + a connect in flight — see above. A second
    // _stopLiveVideo() here would be dead code since the first call cleared the
    // flag.)
  ...
  _updateSchedulesSection(hass, ents) {
    // issue #45 follow-up scan (realKim-dotcom 2026-07-13): was hardcoded
    // German weekday abbreviations regardless of hass.language.
    const WEEKDAY_NAMES = [
      this._t("weekday_sun"), this._t("weekday_mon"), this._t("weekday_tue"),
      this._t("weekday_wed"), this._t("weekday_thu"), this._t("weekday_fri"),
      this._t("weekday_sat"),
    ];
      ...
      if (rules.length === 0) {
        ...
        if (this._lastRulesKey !== rulesKey) {
          ...
          rulesListEl.innerHTML = rules.map((r, i) => {
            ...
            // issue #45 follow-up scan (realKim-dotcom 2026-07-13): was
            // hardcoded German "AN"/"AUS" regardless of hass.language.
          ...
          rulesListEl.querySelectorAll(".rule-toggle").forEach(btn => {
            btn.addEventListener("click", (e) => {
              ...
              // Optimistic UI. issue #45 follow-up scan (realKim-dotcom
              // 2026-07-13): was hardcoded German regardless of hass.language.
  ...
  _showCreateRuleDialog() {
    return new Promise((resolve) => {
      ...
      // issue #45 follow-up scan (realKim-dotcom 2026-07-13): entire dialog
      // was hardcoded German regardless of hass.language.
      overlay.innerHTML = `
      ...
      btn.addEventListener("click", async () => {
        ...
        if (svc.svc === "_trigger_siren") {
          ...
          if (sirenEntity && this._hass.states[sirenEntity]) {
            ...
            if (resultEl) { resultEl.style.display = ""; resultEl.textContent = this._t("siren_triggering"); }
          } else {
            if (resultEl) { resultEl.style.display = ""; resultEl.textContent = this._t("siren_unavailable"); }
          }
  ...
  _waitForStreamReady(attempt = 0) {
    ...
    if (attempt > 90) {
      // 90s elapsed without the camera reporting "streaming". Don't dead-end on a
      // stale snapshot with no recovery: drop the spinner, resume snapshot refresh
      // (so the _update() auto-start gate re-fires on the next hass push), and
      // schedule ONE delayed re-poll in case the backend was only transiently
      ...
      // 30s timer → doubling the poll rate each round. Cleared here before re-arming
      // and in _stopLiveVideo. 2026-06-22 (verify-agent: timer-stack fix).
      if (this._waitForStreamRetryTimer) clearTimeout(this._waitForStreamRetryTimer);
      this._waitForStreamRetryTimer = setTimeout(() => {
        this._waitForStreamRetryTimer = null;
        if (!this.isConnected) return;
        if (this._isStreaming && this._isStreaming() && !this._liveVideoActive
            && !this._startingLiveVideo && !this._waitingForStream && !this._reconnectingLiveVideo) {
          this._waitingForStream = true;
          this._waitForStreamReady();
        }
      }, 30000);
  ...
  _toggleAudio() {
    ...
    if (!entityId) return;
    // Direction: when streaming, toggle the ACTUAL audibility (a video muted by
    ...
    // Drive the backend switch.<cam>_audio on EVERY tap so the mute/unmute choice
    // syncs to every device + automations: HA pushes it back and _update()
    // applies it to the other cards' <video> live (mute reliably, unmute
    // best-effort under their own autoplay policy). 2026-06-01.
  ...
  // True when the given <video> is currently owned by a native OS presentation
  // surface the user is actively watching RIGHT NOW: the W3C Picture-in-Picture
  // floating window (`document.pictureInPictureElement`, Chrome/desktop Safari),
  // the iOS-webkit PiP mirror (`_boschPipActive` — iOS uses
  // webkitSetPresentationMode("picture-in-picture") instead of the W3C API, so
  // `document.pictureInPictureElement` never reflects it there), OR Safari's
  // native <video> fullscreen presentation mode
  // (`video.webkitPresentationMode === "fullscreen"`). iOS Safari can drive a
  // <video playsinline> into this mode on its own (e.g. on landscape rotation)
  // WITHOUT us ever calling webkitEnterFullscreen() — our own fullscreen button
  // uses `_enterCssFullscreen()` on iOS precisely to AVOID native fullscreen
  // (see `_requestFullscreen()`), so that path never touches
  // `webkitPresentationMode` and is NOT covered by this check; only iOS's own
  // native gesture is. Tearing srcObject / video.load() (as `_stopLiveVideo()`
  // does during recovery) while ANY of these own the element corrupts the same
  // underlying WebKit/Chrome compositor surface link (crbug 894317) that PiP
  // depends on — so a recovery cycle that runs during native fullscreen can
  // leave a LATER PiP request stuck spinning forever, even after the user has
  // already left fullscreen (HA-X? iOS PiP-after-fullscreen bug, 2026-07-13).
  // `webkitPresentationMode` is undefined on non-WebKit browsers, so the
  // comparison is always false there — no cross-browser crash risk. This is a
  // live read (nothing cached), matching the existing `ownsPip` pattern.
  ...
  _enterCssFullscreen() {
    ...
    // Tap anywhere outside the image to exit. Shadow-DOM-aware: this.contains()
    // breaks when the card is nested inside the overview card's shadow root —
    // e.target then retargets to the OVERVIEW host (an ancestor, not contained),
    // so a pinch / tap ON this card's own video read as "outside" and exited
    // fullscreen (mobile pinch-zoom kept closing it — Thomas). composedPath()
    // lists every node the event crossed, across shadow boundaries.
    this._fsClickOut = (e) => {
  ...
  _exitCssFullscreen() {
    ...
    if (this._fsClickOut) { document.removeEventListener("pointerup", this._fsClickOut); this._fsClickOut = null; }
    if (this._fsKeyDown)  { document.removeEventListener("keydown", this._fsKeyDown);  this._fsKeyDown  = null; }
    this._resetZoom(); // drop any digital zoom when leaving fullscreen
  }
  ...
  _applyZoom() {
    ...
    for (const id of ["cam-video", "cam-img"]) {
      const el = this.shadowRoot?.getElementById(id);
      if (el) { el.style.transform = t; el.style.transformOrigin = "center center"; }
    }
  }

  _resetZoom() {
    this._zoom.scale = 1; this._zoom.tx = 0; this._zoom.ty = 0; this._zoom.prevDist = -1;
  ...
  // Adjust pan so the zoom anchors at point (px,py) (element coords) when the
  // scale changes from oldScale to newScale (transform-origin is center).
  ...
  _wireZoom(wrap) {
    ...
    const onDown = (e) => {
      ...
      if (this._zoomPointers.size === 1) {
        const now = Date.now();
        if (now - this._zoom.lastTap < 300) {       // double-tap → toggle 2× at point
          e.preventDefault();
          const p = rectPt(e);
          if (this._zoom.scale > 1) this._resetZoom();
          else this._zoomAnchor(wrap, p.x, p.y, 1, 2);
    ...
    const onMove = (e) => {
      ...
      if (pts.length === 2) {                        // pinch
        ...
        z.prevDist = dist;
        e.preventDefault();
      } else if (pts.length === 1 && z.scale > 1) {  // pan
...
class BoschCameraCardEditor extends HTMLElement {
  ...
  set hass(hass) {
    this._hass = hass;
    if (!this.shadowRoot) return;
    // HA pushes hass on every state change. Re-render only when something the
    // editor actually shows changed (UI language or the camera list) — a blind
    // re-render resets the title <input> and steals focus while the user types.
  ...
  _render() {
    ...
    cams.sort();
    // name = stable HTML control name (NOT derived from the label, which is now
    // translated — a DE/EN label swap would otherwise change the selector).
...
class BoschCameraOverviewCard extends HTMLElement {
  ...
  _renderLanTiles(coveredEntityIds = new Set()) {
    ...
    for (const t of tiles) {
      ...
      status.textContent = lanState === "on" ? this._t("lan_reachable")
        : lanState === "off" ? this._t("lan_not_reachable")
        : this._t("lan_status_unknown");
      ...
      const addBtn = (label, entity, domain) => {
        ...
        btn.title = !reachable ? this._t("lan_cam_not_reachable")
                  : !entOk ? this._t("lan_status_unknown_cloud")
                  // issue #45 follow-up scan (realKim-dotcom 2026-07-13): was
                  // hardcoded German "{label} AN/AUS schalten" regardless of
                  // hass.language.
                  : this._t(isOn ? "lan_turn_off_title" : "lan_turn_on_title").replace("{label}", label);
  ...
  _renderMaintenanceBanner() {
    ...
    sub.textContent = win
      ? `${mAttr.title || this._t("maint_notice")} · ${win}`
  ...
  _formatWindow(startIso, endIso) {
    ...
    try {
      ...
      // issue #45 follow-up scan (realKim-dotcom 2026-07-13): date/time was
      // always formatted in de-DE regardless of hass.language — same class of
      // bug already fixed for the offline overlay's "last seen" line.
      const loc = cardLang(this._hass);
      const date = s.toLocaleDateString(loc, { weekday: "short", day: "2-digit", month: "2-digit", year: "numeric" });
      const fmt = (d) => d.toLocaleTimeString(loc, { hour: "2-digit", minute: "2-digit" });
      ...
      if (cams.length === 0) {
        ...
        if (unavailableBosch.length > 0) {
          ...
          if ((mState === "active" || mState === "scheduled" || mState === "recent") && mAttr.camera_relevant) {
            // Best case: Bosch announced exactly this window. (issue #45 follow-up:
            // this empty-state path duplicated _renderMaintenanceBanner()'s copy as
            // raw hard-coded German instead of routing through the same _t() keys.)
            titleEl.textContent = mState === "active" ? this._t("maint_title")
              : mState === "scheduled" ? this._t("maint_title_planned")
              : this._t("maint_title_recent");
            const win = this._formatWindow(mAttr.scheduled_start, mAttr.scheduled_end);
            sub.textContent = win
              ? `${mAttr.title || this._t("maint_notice")} · ${win}`
              : (mAttr.title || this._t("maint_notice"));
            ...
            sub2.textContent =
              `${unavailableBosch.length} ${unavailableBosch.length === 1 ? this._t("outage_cam_singular") : this._t("outage_cam_plural")} ` +
              this._t("outage_auto_return_suffix");
          } else {
            ...
            sub.textContent =
              `${unavailableBosch.length} ${unavailableBosch.length === 1 ? this._t("outage_cam_singular") : this._t("outage_cam_plural")} ` +
              this._t("outage_waiting_for_server");
...
class BoschCameraOverviewCardEditor extends HTMLElement {
  ...
  // HA pushes hass onto the editor element; store it so labels can localise.
  // Re-render only on a UI-language change (this editor has no camera picker) —
  // a blind re-render on every hass push would steal focus from its inputs.
  set hass(hass) {
    this._hass = hass;
    if (!this.shadowRoot) return;
...
class BoschNotificationsCard extends HTMLElement {
  ...
  _maintenanceBanner(maint) {
    ...
    // issue #45 follow-up scan (realKim-dotcom 2026-07-13): was always de-DE
    // regardless of hass.language.
    const loc = cardLang(this._hass);
