...
async def try_live_connection_inner(
    ...
    try:
        ...
        if conn_type_pref == "local":
            ...
            # LOCAL by then. Cf. _LOCAL_RESCUE_TTL_SEC / local_rescue_at.
        ...
        # early-invalidated. ensure_valid_token() is built for exactly this
            ...
            try:
                ...
                if resp.status in (200, 201):
                    ...
                    if type_val == "LOCAL" and local_user and local_pass:
                        ...
                        _session.warming_started = time.monotonic()
                        # Fresh attempt — clear any "ready" signal left over
                        # from a previous session so a waiter (recorder.py)
                        # can't mistake a stale signal for this one.
                        ...
                        coordinator.stream_warming.discard(cam_id)
                        # Signal any waiter (recorder.py's start_recorder)
                        # that rtspsUrl is now genuinely usable — see
                        # session_state.py's stream_ready_event docstring.
