...
- **Proactive maintenance round** (beta-4, Thomas: "mache eine schöne Maintenance Runde") — a live Terrassenkamera stream start/stop cycle plus a broad multi-agent bug-hunt sweep across the card frontend and the FCM/NVR backend surfaced five further real, previously-unreported bugs, all fixed and independently bug-hunted before shipping:
