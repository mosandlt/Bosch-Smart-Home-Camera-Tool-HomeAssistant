...
value. This is intentional: `per_cam_status_at[cam_id]` was already
updated inside a real check, so re-deriving "was anything checked" from
`should_check_status` again here would always read False for
...
async def _check_one_camera_status(
    ...
    # SESSION_LIMIT is NOT a connectivity failure — do not add to offline_since
...
async def poll_statuses(
    ...
    for result in status_results:
        ...
        # Mark as checked unconditionally — per_cam_status_at[cid] was
        # just updated inside _check_one_camera_status, so re-calling
        # should_check_status would always return False for
