...
class FrigateCoordinatorMixin:
    ...
    reads/writes many coordinator attributes directly (`frigate_runner`,
    `live_connections`, `tls_proxy_ports`, `data`, `options`, `hass`, …)
    and calls several coordinator methods (`try_live_connection`,
    `has_active_consumer`, `tear_down_live_stream`, `get_session`,
    ...
    def _frigate_on_idle(self: Any, cam_id: str) -> None:
        ...
        async def _maybe_teardown() -> None:
            ...
            # Capture the generation at decision time: `tear_down_live_stream`
