...
def _record(
    msg: str, logger_name: str = "homeassistant.components.camera"
) -> logging.LogRecord:
...
class TestPlayStreamServiceRateLimit:
    ...
    def test_second_occurrence_within_30s_is_suppressed(self):
        f = _make_filter()
        rec = _record(
            "Error requesting stream: camera.bosch_innenbereich does not support play stream service"
        )
    ...
    def test_non_bosch_entity_is_never_suppressed(self):
        f = _make_filter()
        rec = _record(
            "Error requesting stream: camera.generic_cam does not support play stream service"
        )
        assert f.filter(rec) is True
    ...
    def test_after_30s_window_passes_again(self):
        f = _make_filter()
        rec = _record(
            "Error requesting stream: camera.bosch_kamera does not support play stream service"
        )
        f.filter(rec)
    ...
    def test_max_tracked_prunes_oldest_entry(self):
        ...
        rec = _record(
            "Error requesting stream: camera.bosch_new does not support play stream service"
        )
        f.filter(rec)
        assert len(f._last_passed) <= f._MAX_TRACKED
...
class TestInstallIdempotent:
    ...
    def test_double_install_adds_only_one_filter(self):
        ...
        cam_logger.filters = [
            ...
            if not isinstance(f, _StreamSupportNoiseFilter)
        ]
        _install_stream_support_noise_filter()
        _install_stream_support_noise_filter()
        bosch_filters = [
            f for f in cam_logger.filters if isinstance(f, _StreamSupportNoiseFilter)
        ]
