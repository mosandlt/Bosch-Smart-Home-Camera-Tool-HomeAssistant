...
@pytest.mark.asyncio
async def test_original_replace_error_propagates_when_cleanup_also_fails(
    tmp_path: Path,
) -> None:
    """Secondary-failure branch: replace() AND the .tmp cleanup both raise.

    The cleanup-failure OSError must be swallowed (logged, not raised) so the
    original replace() error — the one the caller actually needs to see — is
    what propagates, not the secondary unlink failure.
    """
    from custom_components.bosch_shc_camera import snapshot_store
