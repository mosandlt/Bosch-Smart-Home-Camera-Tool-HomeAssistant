...
- **SMB share/server/username/password still could not be cleared after beta-2 (round 3).** Root cause, this time verified against HA-frontend's actual `ha-selector-text` source rather than inferred: the component converts an emptied field's value to `undefined` on every change (`if (value === "" && !this.required) value = undefined`), and `undefined` fields are dropped entirely from the submitted data — a genuinely-cleared field and a never-populated one are indistinguishable "missing key" cases once they reach the integration's Python code. beta-2's *dynamic* `default=opts.get(...)` (the persisted value) resolved that missing-key ambiguity the wrong way, resurrecting the old value on every save regardless of what the user did — the opposite of round-1's problem, but the same underlying inability to tell "untouched" from "cleared" apart at the schema-default layer. A *populated* field's value is never converted to `undefined` (only an emptied one is), so it's always included in the submission independent of the schema default — meaning a **static** `default=""` (not tied to the persisted value) is safe for the untouched/populated case and correct for the missing-key case. Fixed by switching all four fields to `default=""`, which fixes the natural "clear the box and save" workflow directly. Because two prior rounds already failed live, also added an explicit `clear_smb_credentials` checkbox to the same section as a second, unambiguous way to force a clear — a boolean selector's checked state is always a real `true`/`false`, never `undefined`, so it cannot hit this class of bug at all.
- **Automations calling `open_live_connection` got `Unauthorized`, and Mini-NVR recording intent could be lost permanently across a restart** (GitHub [#71](https://github.com/mosandlt/Bosch-Smart-Home-Camera-Tool-HomeAssistant/issues/71), realKim-dotcom). The v16.1.15 admin gate rejects any service call with no resolvable admin `user_id` — correct for account-level/destructive services, but every HA automation/script context carries `user_id: None`, so the documented way to keep a 24/7 Mini-NVR pre-roll ring alive (an automation calling `open_live_connection` then `switch.turn_on` on the recording switch) failed on every run: no LOCAL session ever opened, the recording switch stayed `unavailable`, and `switch.turn_on` was silently skipped by HA on an unavailable entity. Compounding it, `BoschNvrRecordingSwitch` only restored its ON intent from `RestoreEntity` when the last rendered state was literally `"on"` — once the switch had been `unavailable` at any restart, that intent was lost with no way back short of manually re-toggling the switch. Fixed in two parts: `open_live_connection` is no longer admin-gated (same non-destructive-session-action precedent as the existing `trigger_snapshot` exemption — it only opens a proxy stream connection, nothing account-level or destructive); and Mini-NVR recording intent is now persisted to its own `Store`, independent of `RestoreEntity`, loaded before platforms are set up so it survives a restart even when the switch itself was `unavailable` at shutdown.
  - Three parallel opus bug-hunt agents on the initial two-part fix all independently converged on the same real gap: the switch's own `RestoreEntity` restore path set the intent in memory but never persisted it to the new Store, so an install upgrading from before this fix could still lose intent on a second restart — the exact symptom being fixed. Closed by persisting immediately on restore. Two agents separately found the Store was never cleaned up on camera removal (a removed camera's stale `True` entry would resurrect a phantom session on the next restart, now purged alongside the rest of `_purge_cam_id`'s per-camera state) or on full integration removal (`async_remove_entry` deleted four Store files but not this new one — removing and re-adding the same account could silently resurrect recording; now included).
  - 6895 pytest / mypy --strict / ruff / codespell clean.
...
- **New Repairs issue: "NVR enabled but not recording" (GitHub #64/#70 follow-up).** Investigated a report that local event recording produced nothing despite the media folder being correctly linked. Root cause: the "Enable NVR" integration option only creates the per-camera NVR entities (`select.*_nvr_mode`, `switch.*_nvr_recording`, `switch.*_nvr_event_clips`) — all `entity_registry_enabled_default=False` (hidden by default) — and does not itself start recording. A user who enables the global option but never finds and enables/activates those hidden entities got zero recordings with zero visible error. A new entry-wide Repairs issue now fires when `enable_nvr` is on but no camera is actually recording, pointing the user at the specific entities to enable — including flagging `event_buffered` mode cameras whose global Preroll/Postroll seconds options are both still at their `0` default (which alone silently prevents any clip from ever being captured, even with the switch on).
  - A 3-agent adversarial bug-hunt on the initial implementation found two real bugs before release: (1) a false positive on every HA restart, since `coordinator.data` is populated before the switch/select platforms are forwarded and restore their `RestoreEntity` state — fixed with a 5-minute debounce (`NVR_NOT_RECORDING_GRACE_SEC`), same idiom as the existing `CAMERA_OFFLINE_ANNOUNCE_GRACE_SEC`; (2) the initial check only looked at the recording switch's intent, missing the `event_buffered` + zero-preroll/postroll case above — the single most likely real-world cause of the original report — now covered.
  - 6879 pytest / 100.00% coverage / mypy --strict / ruff / codespell clean.

## [v16.2.1] - 2026-09-04

- **FCM device-token backend-load fix: some HA instances were still registering >30 new FCM device tokens/day.** Despite v16.1.10/v16.1.11, root cause found in `_async_run_fcm_supervisor()`: the benign "soft-restart-threshold" hard-heal trigger (3 consecutive listener restarts with no push received) purges FCM credentials and mints a brand-new token on every occurrence, and was deliberately excluded from the existing CONFIRMED-problem backoff ladder — on a persistently flaky WAN/router this benign path can recur every 1.5-5 min indefinitely, each time purging good credentials and re-registering. Fixed with a SEPARATE, lighter backoff (`soft_trigger_heal_streak`, same `FCM_SUPERVISOR_BACKOFF_SEC` ladder) specifically for this trigger — reset by either `FCM_SUPERVISOR_SUSTAINED_UPTIME_SEC` (600s) of continuous listener uptime OR a received push (whichever comes first), so a genuinely-recovering connection is never penalized, while a truly flaky one now backs off exactly like the CONFIRMED-problem path already does. Also wired in a `PUT v11/registration/logout?deviceToken=<token>` endpoint (new `deregister_fcm_with_bosch()`) — called best-effort, right before every hard-heal purge discards the currently-registered token, so the backend no longer accumulates an abandoned registration per purge.
  - Two independent opus bug-hunt agents converged on the same real gap during review: the new streak was reset by sustained uptime but NOT by a received push (a strictly stronger signal delivery works), which could have let the backoff climb toward its 30-minute ceiling in a house with working push delivery but a chronically restarting listener — fixed before release. A third finding broadened `deregister_fcm_with_bosch`'s exception handling from `(TimeoutError, aiohttp.ClientError)` to a bare `Exception`, matching its own "never raises" docstring promise — the narrower version could have let an unexpected error escape into the hard-heal purge's own exception handler, aborting the ENTIRE purge (not just the best-effort cleanup step) and stalling credential recovery in a 5s retry loop.
  - 6865 pytest / 100.00% coverage / mypy --strict / ruff / codespell clean.

## [v16.2.0] - 2026-08-23

- **New opt-in feature: live camera preview in alert push notifications (iOS only).** New `alert_notify_live_preview` option (off by default, under Configure → Push notifications): when ON, the FCM alert pipeline resolves the camera's real HA entity_id via the entity registry (same unique_id scheme `camera.py` itself registers) and adds `entity_id: camera.bosch_<name>` to every `mobile_app_*` alert notify payload, alongside the existing static snapshot — the Companion App's own dynamic-content mechanism (the same one HA's notification docs document for `entity_id:`). The app fetches its own fresh thumbnail on delivery and opens a live camera feed inside the notification when the recipient expands it; the static snapshot stays attached and still shows instantly (additive, not a replacement). Android's notification attachments only support a static image, not this live-stream mechanism, so today this only helps iOS. Only applies to `mobile_app_*` services; Signal/Telegram/email and other notify targets are unaffected. Deliberately skipped for `TROUBLE_CONNECT`/`TROUBLE_DISCONNECT` connectivity alerts (no meaningful live feed to offer, and DISCONNECT specifically means the camera is unreachable) and when `enable_snapshots` is OFF (no camera entity exists to resolve). Off by default because it makes the Companion App query the camera on every alert delivery for every recipient device, not only on expand. A 3-agent adversarial bug-hunt on this change caught and fixed 3 real issues before release: an initial version of the wording claimed the live view uses WebRTC — HA's Companion docs and the actual iOS app source (`CameraViewController`/`NotificationAttachmentParserCamera`) show no WebRTC path in the notification content extension, corrected across all 12 locales; enabling the option was silently adding a `push.sound` key to step-1 text-only alerts that never had one before — removed, this option only adds a preview; and the live-preview `entity_id` was being attached to `TROUBLE_CONNECT`/`TROUBLE_DISCONNECT` alerts with no meaningful use for it — now gated out. 6856 pytest / 100.00% coverage / mypy --strict / ruff / codespell clean, all 12 locales complete.

## [v16.1.15] - 2026-08-21

- **Round 2 of the hacs/default#8181 security review (frenck): admin-gate correctness fix + a second unconditional instance-wide side effect closed.** The TLS-pinning and random-filename fixes from v16.1.14 were accepted as-is. Two points needed more work:
  - **The admin gate didn't actually fail closed.** v16.1.14's `_require_admin()` treated a missing `context.user_id` (which an unauthenticated/context-less caller also produces) as an implicitly-trusted automation call and let it through — exactly the case the maintainer's original ask meant to cover. Fixed: `guards.async_require_admin()` now rejects unconditionally — no context, no user_id, an unresolvable user_id, or a non-admin user are ALL rejected, deliberately stricter than HA core's own `helpers.service.async_register_admin_service` (which does treat a missing user_id as trusted). Also extended the gate from just the two switch entities (panic-alarm, intrusion arm/disarm) to all ~20 custom services this integration registers (`share_camera`, `invite_friend`, `remove_friend`, `delete_event`, the `create_rule`/`update_rule`/`delete_rule` set, and the rest, gated for consistency) via a new `guards.admin_only_service()` wrapper. **Exception**: `trigger_snapshot` stayed unwrapped — a 3-agent adversarial bug-hunt on this exact change caught that the Lovelace card calls it automatically on page load, tab switch, and a 60s timer for every viewer, so admin-gating it would have broken the card's live-image refresh for any non-admin household member; it's a read-only refresh trigger, not an account-level action.
  - **`cf_unbuffer.py`'s HLS view monkeypatch is now opt-in** (new `cloudflare_tunnel_hls_unbuffer` option, default OFF, config-flow description explains exactly what it does and its instance-wide scope) instead of applying unconditionally at every setup. The same bug-hunt caught that a naive "just gate the whole `register()` call" implementation would have silently broken idle-session detection for the DEFAULT (option-off) case: `cf_unbuffer`'s wrappers are the *only* source for `hls_access_age()`, which `stream_lifecycle.has_active_consumer()` reads to detect an actively-watched HLS viewer — gating installation itself would have made `idle_session_reaper` tear down a session someone is actively watching over HLS, for every install that doesn't opt into the Cloudflare-Tunnel workaround. Fixed by splitting the two concerns: wrapper *installation* (and HLS-access stamping) stays unconditional; only the actual Content-Type/Transfer-Encoding *rewrite* is gated behind the new option.
  - 6843 pytest / 100.00% coverage / mypy --strict / ruff / codespell clean, all 11 translation locales complete.

## [v16.1.14] - 2026-08-20

- **Security hardening from the HACS default-repo review (hacs/default#8181, maintainer frenck): 3 real findings fixed.**
  - **Cloud TLS proxy no longer accepts an arbitrary certificate.** The Python TLS proxy that terminates REMOTE (cloud) RTSPS sessions previously used a single shared SSL context with `verify_mode=CERT_NONE`/`check_hostname=False` for both the LOCAL self-signed camera connection and the public-internet Bosch cloud video proxy (`proxy-NN.live.cbs.boschsecurity.com`). That meant an attacker able to intercept the cloud connection could present literally any certificate, including a self-signed one, and the proxy would accept it — live video from someone's home flowing through an unauthenticated TLS tunnel. LOCAL and CLOUD now get independently cached SSL contexts: LOCAL is unchanged (a self-signed cert is unavoidable for the camera's own LAN certificate); CLOUD now requires `CERT_REQUIRED` anchored to the same private Bosch CA (`cloud_ssl.BOSCH_CLOUD_CA_PEM`, `Bosch ST Root CA` → `Video CA 2A`) this integration already pins for the REST API — Bosch's cloud/video-proxy hosts are served by a private PKI not present in any public trust store, so a bare system-trust-store context (the initial version of this fix) would have rejected Bosch's own genuine certificate and broken every REMOTE stream; a 3-agent adversarial bug-hunt independently caught this before release, each agent verifying live via a real TLS handshake against `residential.cbs.boschsecurity.com` that the private-CA-pinned context succeeds and a bare system-trust-store context fails with `CERTIFICATE_VERIFY_FAILED`. `check_hostname` stays off — Bosch's proxy-NN hostname legitimately doesn't match its own certificate's SAN (`*.residential.connect.boschsecurity.com`), independently confirmed by go2rtc's Go RTSP client refusing the same certificate with `tls: failed to verify certificate` when hostname checking is attempted — but the certificate must still chain to Bosch's own CA, closing the actual gap: an attacker now needs a certificate signed by Bosch's private CA, not literally any certificate.
  - **Alert-notification image/clip filenames are no longer guessable.** Motion/person-event snapshots and video clips are written to `config/www/bosch_alerts/`, served unauthenticated at `/local/bosch_alerts/` — required because HA's mobile_app push-notification image is fetched by the OS's push handler outside any Home Assistant session/auth context, so it cannot live behind an authenticated view. The filename was previously just `{camera_name}_{timestamp}_{event_type}`, guessable/enumerable by anyone who knew (or could infer) a camera's name and roughly when an event fired. Every alert now gets a 128-bit random token (`secrets.token_urlsafe(16)`) baked into both the `.jpg` and `.mp4` filename, on top of the existing components — HA's static file route exposes no directory listing, so without the token there is nothing left to guess or enumerate.
  - **Panic-alarm (siren) and intrusion-system arm/disarm now require an admin user.** Neither service previously checked the calling user's permission level beyond HA's normal per-entity access grant — any non-admin household member (or a narrowly-scoped/compromised non-admin token) with access to the entity could trigger the physical 75dB siren or disarm the intrusion system. Both now reject a non-admin (or unresolvable) calling user with `Unauthorized`, fail closed. Automations/scripts (which carry no calling-user context) are unaffected — HA already gates who may author those.
  - Full detail: 3 independent opus bug-hunt agents reviewed the fix before release; all three independently caught the cloud-TLS system-trust-store gap above pre-release (never shipped). 6834 pytest / 100.00% coverage / mypy --strict / ruff / codespell clean.

## [v16.1.13] - 2026-08-19

- **FCM push subsystem hardening: 25 real bugs fixed after a 5-agent exhaustive bug-hunt across the entire `fcm.py` push-client/supervisor/dispatch/alert pipeline.** Following the v16.1.12 header-extraction fix, five agents swept the whole subsystem end to end and found 25 independently-confirmed issues, all fixed with regression tests. Highlights:
  - **Silent event loss after every HA restart in any non-UTC timezone**: `_event_predates_session` truncated Bosch's timestamp before parsing, discarding the timezone offset and re-labeling local wall-clock time as UTC — in CEST, the first real motion event after any restart was computed as 2 hours in the past and silently dropped as "stale". Now uses the existing `parse_bosch_timestamp()` helper (same latent bug fixed in `smb.py`/`recorder.py` too).
  - **Alerts firing despite the master notifications switch being OFF**: the notification-switch lookup hand-rebuilt an entity_id from the camera name with only partial special-character handling instead of a real entity-registry lookup — a privacy-relevant fail-open bug on non-trivial camera names/locales.
  - **Stale credentials could silently undo a hard-heal**: a race between the FCM client's credential-persist callback and the supervisor's hard-heal purge could let stale/rejected credentials get rewritten right after a purge, explaining why FCM push recovery "sometimes worked, sometimes didn't" after a registration failure.
  - **AI-description timeout could silently swallow the whole screenshot alert**: the optional AI-description append shared the same 15s timeout budget as the image download despite needing up to 20s itself, and a slow AI call's cancellation escaped the narrow exception handler meant to isolate it — the already-downloaded snapshot then leaked on disk (never cleaned up) while being served unauthenticated. AI description now runs in its own independent timeout window; cleanup registration is now unconditional.
  - Unbounded video-clip download (no size cap), truncated `.mp4` files on SMB shares becoming permanently un-repairable (now written to a temp name and renamed only on success), events getting marked read on Bosch's side even when zero notification steps actually succeeded (silent permanent loss), a null-writer crash in the FCM client's message-skip path during an in-flight connection reset, and several supervisor lifecycle gaps (undrained clients leaking a background task on every failed start, the polling-mode supervisor hard-healing forever against a mode the user deliberately chose, a forced hard-heal silently dropped if the credential purge itself raised).
  - Full list of all 25 findings + fixes in the release commit body. 6824 pytest (was 6754), 100.00% coverage maintained, mypy --strict / ruff / codespell clean, all 11 translation files verified complete.

## [v16.1.12] - 2026-08-19

- **Fixed FCM push messages being silently dropped or crashing the push client with "Invalid EC key."** Root cause: the bundled `firebase_messaging` library blindly slices the per-message `crypto-key`/`encryption` headers (`header[3:]`/`header[5:]`) assuming they always start with exactly `"dh="`/`"salt="` and carry no other content. Real-world headers can be multi-segment (`dh=<key>; p256ecdsa=<vapid>`, RFC 8291-legal) or differently ordered/cased, corrupting the extracted key bytes just enough to still base64-decode but fail EC-point validation — surfacing as intermittent "Invalid EC key." (sometimes crashing the whole push client, first observed live 2026-08-18; sometimes just silently dropping one push after the 2026-08-18 hardening made the client survive it). Added a corrected `_handle_data_message` override (matching upstream sdb9696/firebase-messaging#42 + #44, both open/unmerged as of 2026-08) alongside the existing padding/decrypt overrides, wired into the same signature-guarded monkey-patch pattern. A 4-agent adversarial bug-hunt on the initial fix found and closed 4 real gaps before release: segment-order dependence (a VAPID/RFC-8188 parameter preceding `dh=`/`salt=` silently returned the wrong value), a case-sensitivity regression versus the old behavior, leftover whitespace after prefix removal throwing off the padding math, and a signature guard that only checked the patched method's own signature rather than the six other private upstream methods its replicated body depends on. Live-verified end to end on real hardware: a genuine Bosch motion push at 22:08:20 decrypted and delivered cleanly (text alert + screenshot) with zero "Invalid EC key." skip, confirming the fix against real production traffic, not just synthetic test cases. 21 new regression tests, 100% coverage on the new code, full suite (6754 tests) green.

- **Removed the `unread_events_count` sensor and its backing endpoint.** Bosch's own Camera Backend team reported that `GET /v11/video_inputs/{id}/unread_events_count` — polled once per camera on every slow-tier (~5-min) cycle for every installation — does not exist in their API at all. The sensor was `entity_registry_enabled_default=False` (disabled by default) and, since the endpoint could only ever 404, its backing cache field could never have been populated in any installation. Fully removed: the endpoint fetch and its dispatch branch (`slow_tier.py`), the cache field (`coordinator.py`), the `BoschUnreadEventsCountSensor` entity/description (`sensor.py`), and the `unread_events` translation key from `strings.json`/`icons.json`/all 11 locale files. THREE_PER_ISSUE_PER_CHANGE 3-agent adversarial bug-hunt found one real (cosmetic) issue — a stale docstring in `slow_tier.py` still advertising the removed endpoint — fixed; confirmed no orphaned references, no entity-registry migration needed (matches this codebase's existing precedent for other never-functional entities, e.g. v13.2.0's audioAlarm cleanup, which also shipped with no registry-cleanup pass since the entity was always disabled-by-default and effectively invisible), and no test/type/JSON-structure regressions. 6733 pytest / mypy --strict / ruff / codespell clean, 100% coverage.
- **Investigated a second Bosch Camera Backend report: `POST /v11/devices` (FCM device-token registration) called very frequently since 2026-07-28**, hypothesized by Bosch as multiple FCM tokens being generated instead of one being reused. Root-caused: every FCM supervisor hard-heal purges ALL `fcm_*` entry-data keys, including the Firebase library's own persisted credentials — this is intentional (a hard-heal IS a credential purge + fresh registration, the correct response to a genuinely stale/rejected credential), but it also means `checkin_or_register()` mints a genuinely new FCM token on every hard-heal, which then legitimately can't be deduped against the previous registration. Confirmed no code changed in `fcm.py` anywhere near the 2026-07-28 onset date Bosch flagged — the mechanism (and the bug that let hard-heals repeat without backoff, causing token churn) predates that date and was already fixed by the `v16.1.10` (2026-08-17/18) backoff-escalation work: hard-heals for CONFIRMED problems now back off exponentially instead of retrying immediately, which directly bounds how often a new token — and therefore a new `POST /v11/devices` — can be generated. No further code change required for this report.

## [v16.1.10] - 2026-08-17

- **FCM push registration could get stuck in a recurring `PHONE_REGISTRATION_ERROR` loop against Google's registration endpoint** (GitHub [#68](https://github.com/mosandlt/Bosch-Smart-Home-Camera-Tool-HomeAssistant/issues/68)). When the FCM supervisor's hard-heal (credential purge + fresh registration) didn't actually restore push delivery, it kept retrying at the same near-immediate cadence indefinitely — a registration storm that can itself look like abuse and get the client throttled further. The supervisor now backs off with an escalating delay across consecutive hard-heals that fail to bring back a push, but only for CONFIRMED-problem triggers (the delivery-death watchdog, or a real `PHONE_REGISTRATION_ERROR`) — a benign trigger (an ordinary WAN-blip threshold, or a fresh install with no credentials yet) never feeds this backoff, since a quiet house with no motion has no push to prove a benign heal actually worked. Also fixed a separate, previously-dormant bug found investigating the same issue: the log-noise filter that records `PHONE_REGISTRATION_ERROR` as a "creds stale" signal was only ever installed on `firebase_messaging.fcmpushclient`, but that error is actually logged by the library's sibling `firebase_messaging.fcmregister` logger — a `logging.Filter` on one logger is never consulted for a sibling's own records, so the staleness signal had silently never worked in production. 3-agent adversarial bug-hunt on the fix found and fixed the quiet-house false-positive above (independently confirmed by two of the three) plus the dormant logger-filter bug; also verified no off-by-one in the backoff-ladder indexing, no interaction with the existing external supervisor-restart watchdog, and no compounding with the separate soft-restart backoff ladder.
- **Found the actual root cause of GitHub [#68](https://github.com/mosandlt/Bosch-Smart-Home-Camera-Tool-HomeAssistant/issues/68)'s "notifications always arrive late" report, reproduced live on our own test hardware within the hour** — the pinned `firebase-messaging` PyPI library (0.4.5) has a real bug in `FcmPushClient._decrypt_raw_data`: it decodes the push message's `crypto-key`/`encryption` headers with plain `base64.urlsafe_b64decode`, which raises `binascii.Error` on the RFC-8291-legal *unpadded* base64 those headers actually arrive as. This integration's own earlier fix (issue #65) only caught that error and silently dropped the message, falling back to the ~15-90s coordinator polling path to eventually discover the same event — measured live as a 2:59 delay on a real motion event. There's a real, open, unmerged upstream fix for this exact bug (sdb9696/firebase-messaging#37) that pads correctly so decryption actually succeeds; ported the same fix in as a local override of the `@staticmethod`, deliberately decoupled from the pre-existing issue #33 `_listen` fix so a future unrelated library signature change can only degrade this specific improvement, never the foundational one. THREE_PER_ISSUE_PER_CHANGE 3-agent adversarial bug-hunt (all three independently converging on the same finding) caught one real regression the initial version of this fix introduced: with headers now decoding successfully, a message whose *ciphertext body* is genuinely corrupt (a stale key after a registration rotation, a message meant for a different subtype) now reaches `http_ece.decrypt()` and raises `ECEException` — previously unreachable, since every message failed earlier at the header-padding step first — which would have escaped the existing narrow catch and torn down the whole FCM client over one bad payload; fixed by widening the catch alongside the padding fix. Deploy-verified live on test HA: a real push arrived with `last_push_seconds_ago: 0` immediately after restart, zero `bosch_shc_camera` exceptions. 6717 pytest / mypy --strict / ruff / codespell clean, 100% coverage.
- **Live production incident, same day as the fix above: FCM push crashed and stayed down for ~5 hours** on real test hardware. A single Bosch/Google push message's `crypto-key` header, once correctly padded and base64-decoded, turned out not to represent a valid point on the P-256 curve — `http_ece.decrypt()`'s own EC-point parsing (`derive_dh` → `EllipticCurvePublicKey.from_encoded_point`) raises a raw `ValueError: Invalid EC key.` that, unlike the AEAD/tag-mismatch path, is NOT normalized to `ECEException` — so it escaped the skip-logic added earlier today and tore down the whole `FcmPushClient`. Since the poisoned message was never acked, Google MCS redelivered it on every reconnect, crashing the client repeatedly until diagnosed live in production logs and fixed. THREE_PER_ISSUE_PER_CHANGE 3-agent adversarial bug-hunt on the initial (broader) fix caught a real design flaw before release: a plain `except ValueError` around the whole `http_ece.decrypt()` call would also have caught genuine stored-credential faults (a non-EC or wrong-curve stored key) originating from OUR OWN keys, silently masking them as one-off skippable messages forever instead of triggering the supervisor's hard-heal — exactly the class of silent-push-loss bug #68 was originally about. Fixed with a surgical alternative: pre-parse only the message's own EC point ourselves before calling `decrypt()`, converting just that failure to `ECEException`; genuine credential faults from `load_der_private_key`/`decrypt()`'s other internal calls stay unguarded and still propagate correctly. Also fixed a second real gap the bug-hunt found: the skip path wasn't acking the message (upstream only does that after `_handle_data_message` returns successfully), so even a correctly-skipped poisoned message would have kept redelivering forever — now explicitly appended to `persistent_ids` and selectively acked in the skip path itself, matching what a successful decrypt would have done. Deploy-verified live: FCM push healthy and receiving again immediately after restart, zero `bosch_shc_camera` exceptions. 6720 pytest / mypy --strict / ruff / codespell clean, 100% coverage.
- **Diagnostic-only follow-up**: the same "Invalid EC key" skip recurred twice more after the fix above (~2h apart), harmlessly each time, but the log gave no way to tell whether it was the SAME message still failing to get acked or a genuinely NEW malformed message from Bosch's cloud recurring on its own schedule. The skip-path warning now logs the message's `persistent_id` alongside the error so a future recurrence is immediately diagnosable — a repeating id means the ack isn't durably reaching Google and needs a real fix; distinct ids each time means it's Bosch-side and already fully handled. No behavior change.
- **Exhaustive structured bug-hunt of the whole FCM push subsystem** (25-agent 5-lens sweep + independent verification pass, all 14 distinct findings confirmed real), covering everything the day's incidents hadn't already caught:
  - **Pre-existing bug from before today, likely the actual amplifier behind the day's registration-heavy incidents**: the original issue #33 fix (routing the FIRST connection error to the quiet log path) had a side effect nothing previously caught — upstream's own quiet branch never calls `_reset()`, only its loud branch does, so every routine WAN blip/MCS disconnect now spun on `sleep(1)` forever instead of reconnecting, recovering only via the supervisor's outer teardown + a full fresh Google registration instead of a cheap in-place reconnect. Fixed: both branches now call `_reset()` (idempotent, lock-guarded).
  - **Direct regression of the backoff fix shipped earlier today**: if re-registration kept failing without ever emitting one of the 3 specific "creds stale" marker strings (e.g. a plain WAN outage or FCM-install failure), `confirmed_problem` stayed `False` forever via the "no persisted credentials" reason, bypassing the new backoff entirely and pinning retries at 5s indefinitely. Fixed: that reason is now confirmed once we've already hard-healed at least once this supervisor's lifetime (a genuinely fresh install still stays benign).
  - **Same crash-loop class as the EC-key incident, two more triggers**: a message missing its crypto-key/encryption header entirely (any non-webpush control message, or what an `aes128gcm`-encoded message would look like) raised an unwrapped `RuntimeError` from upstream's header lookup; non-ASCII bytes in a header (RFC 8291-legal) raised an unwrapped `UnicodeEncodeError`. Both escaped the skip logic and would have crashed the client the same way. Fixed: the `RuntimeError` case is matched narrowly (only this specific message shape, not every RuntimeError) and skipped; the `UnicodeEncodeError` case is normalized to the already-skippable `binascii.Error`.
  - **Inverse bug at the same spot**: a corrupt *stored* credential whose base64 length happened to land on `%4==1` also raised `binascii.Error` — which is skippable — so a genuinely corrupt credential got silently skip-and-acked forever instead of triggering hard-heal, permanently degrading to poll fallback with zero self-recovery. Fixed by splitting message-header decoding from credential-material decoding into two helpers with opposite failure-classification.
  - Leaked `aiohttp.ClientSession` on every failed registration attempt (now uses HA's shared session); a caller-directed shutdown cancellation could be silently swallowed instead of propagating as `__init__.py`'s teardown code already assumed; the new `persistent_id` diagnostic log was capped at 5 occurrences forever by an upstream rate limiter (now logged via our own logger); a meaningless empty-string ack could be sent (protobuf default is `""`, not `None`); alert delivery was reported as "sent" even when it silently failed (`hass.services.async_call` now blocks so failures actually propagate); a screenshot/video-only alert configuration (no `information` service) silently sent nothing at all.
  - Plus test-hardening: a new test proves the #68 padding-fix override actually attaches against the *real* installed library (previously only checked against a hand-rolled fake, meaning a future library upgrade could have silently regressed #68 with a green, 100%-coverage suite); the shared test fixture's default now matches production's real config instead of the inverse.
  - A dedicated interaction-focused review of all 14 fixes together (implemented across several independent passes) found no interaction bugs; one minor test gap it flagged was closed in the same round. Deploy-verified live: zero `bosch_shc_camera` exceptions, FCM healthy, a real push received 2s after restart. 6740 pytest / mypy --strict / ruff / codespell clean, 100% coverage.

## [v16.1.9] - 2026-08-16

- **The actual root cause of GitHub [#67](https://github.com/mosandlt/Bosch-Smart-Home-Camera-Tool-HomeAssistant/issues/67)'s "remote stream flapping over a Cloudflare Tunnel" — misdiagnosed in v16.1.8 as an upstream cloudflared GET-buffering limitation.** A REMOTE-mode live stream's connection badge could get permanently stuck on "Verbinde" (connecting) — and, worse than originally understood, this wasn't just a display bug: the same staleness also fed the stall-checker's escalation path, forcing a full reconnect/rebuild loop every ~10s on a stream that was actually decoding fine. The card's `requestVideoFrameCallback` liveness heartbeat only re-armed itself while `video.srcObject` was set (WebRTC playback) — REMOTE sessions mostly fall back to HLS/MSE (`.src`/`currentSrc`, no `srcObject`), so the heartbeat fired once on the first HLS frame and then never rearmed, leaving its "last frame" timestamp stale. ~10s later the freeze watchdog treated that staleness as a real freeze, flipped the badge to "connecting", and (since `frameFrozen` also feeds the stall-checker's fast-escalation branch) triggered a full stream teardown+rebuild — repeating indefinitely, exactly matching #67's "plays ~8-15s → stalls → rebuilds" report. Fixed by also rearming on `video.currentSrc`/`video.getAttribute("src")`, matching an existing identical source-presence check already used elsewhere in the same file. 3-agent adversarial bug-hunt found no issues with the fix; confirmed `requestVideoFrameCallback` fires identically for HLS/MSE `<video src>` playback as for WebRTC, and that every other consumer of the same heartbeat timestamp is either WebRTC-gated or unaffected. Reporter-confirmed fixed same day.

## [v16.1.8] - 2026-08-08

- **Found the actual architectural gap behind GitHub [#64](https://github.com/mosandlt/Bosch-Smart-Home-Camera-Tool-HomeAssistant/issues/64)'s remaining "still no motion clip" reports, after nine prior beta rounds each fixed a real but different bug in the same area: Mini-NVR event_buffered clip assembly was only ever triggered from the FCM push handler.** The coordinator's independent polling path already fires the HA bus event and alert notification whenever it discovers a genuinely new event first — it even has its own "FCM push never delivered this event" delivery-death detector — but never called clip assembly. On an installation where FCM push is slow or unreliable, automations and alerts fire correctly via polling while the native Mini-NVR clip silently never appears, with none of this integration's own NVR diagnostics (added across the prior nine betas) ever getting a chance to run, since all of them live downstream of a gate the polling path never reached. Extracted the gate-and-schedule logic into a shared `maybe_schedule_nvr_motion_clip()` and wired it into both the FCM push handler and the polling path; the existing per-event-ID dedup bookkeeping the two paths already share means only whichever one discovers a given event first ever schedules a clip for it — no double-assembly risk. Also closed two gaps a 3-agent adversarial bug-hunt found in the same change: a poll tick can discover an event long after the pre-roll ring's own retention window has rotated past it (the poll interval can reach 300s while a typical ring keeps ~30s), which would have shipped a clip of unrelated later footage instead of the actual motion — polling-discovered events older than the ring's window are now skipped with an explanatory warning instead; and wiring the existing "clip not created" diagnostic into the polling path made it reachable on every install's first polled motion event, including installs that never touched Mini-NVR at all — now suppressed when the feature is off system-wide. Every diagnostic line now also states which path (`fcm_push` or `polling`) triggered it, since several reporters' log excerpts filtered to lines containing "nvr" and missed the FCM handler's own lines entirely, which made earlier rounds of this issue hard to fully diagnose from the log alone.

- **Found the actual root cause of GitHub [#64](https://github.com/mosandlt/Bosch-Smart-Home-Camera-Tool-HomeAssistant/issues/64): the FCM push handler silently dropped the very first motion/person event for a camera after every Home Assistant restart, with zero log trace at any level — every prior fix in this thread addressed a real but different bug without touching this one.** `async_handle_fcm_push` only dispatched an event once `coordinator.last_event_ids[cam_id]` already held a baseline ID (`prev_id is not None`); that baseline is normally seeded by the coordinator's own polling tick, which deliberately skips event-fetching on the fast ~15s first tick after startup and only runs it ~60-90s later. A real FCM push arriving in that gap — exactly how the reporter reproduced this every single time ("restarted Home Assistant and triggered a motion event") — found `prev_id is None`, so the dispatch condition was false and the event fell into a fallback branch that only silently recorded the ID as a new baseline, without firing the HA event, the alert, or (for Event Buffered mode) the Mini-NVR clip assembly. That fallback logic was correct for the coordinator's own polling path (avoids replaying a backlog of pre-existing events after every restart) but wrong for a push notification, which only ever arrives because Bosch's cloud just observed something happen in real time — copying the same guard onto the push path meant the first genuinely new event after every restart was swallowed, indefinitely, with nothing to show for it. Fixed by dispatching whenever the newest event ID differs from the baseline, regardless of whether a baseline exists yet — with one added guard: if no baseline exists AND the event's own Bosch-cloud timestamp predates this HA session's start (a queued/redelivered push, or the first push after a brand-new install surfacing pre-existing cloud history), it's still skipped, reusing the same session-start staleness check `sync_local_save` already applies elsewhere in the integration. 2-agent adversarial bug-hunt run before release: both independently confirmed the root-cause diagnosis and the fix direction, and one flagged the fresh-install false-positive edge case that's now covered by the staleness guard above.
- **Mini-NVR motion clips still silently failed to appear even after every prior GitHub [#64](https://github.com/mosandlt/Bosch-Smart-Home-Camera-Tool-HomeAssistant/issues/64) fix, with no warning logged and the pre-roll ring confirmed healthy.** `list_preroll_files()` always drops the newest ring segment (it may still be mid-write) — a motion event arriving while only one segment exists on disk (e.g. right after a Home Assistant restart, exactly how the reporter reproduced it every time) therefore sees an empty segment list. That "no usable segments" case, along with a separate "clip assembly already in progress" case, both fell through to a `DEBUG`-only log inside `assemble_and_ship_motion_clip`/`create_motion_clip` — invisible by default, and outside the scope of the existing outer-gate WARNING in `fcm.py` (which only fires when the gate itself is unmet; here it was fully satisfied). Both paths now log a one-time-per-camera WARNING, using two independent flags (a 2-agent adversarial bug-hunt found the first draft shared one flag across both conditions, letting whichever fired first silently suppress the other) cleared as soon as a clip is next successfully shipped. Diagnostic-only change — does not alter clip-creation logic or timing; the next motion event's log should point straight at the remaining cause.
- **Live stream still flapped on a remote connection even after the LL-HLS fix and the reconnect backoff below.** Root cause: `pc.ontrack` (the WebRTC track-arrival handler) had no staleness guard, unlike its sibling handlers `onmute`/`onunmute` a few lines below it (a 2026-06-22 fix), which already check the peer-connection identity before acting. On a slow/lossy connection, an EARLIER WebRTC negotiation attempt — already abandoned in favor of sticky HLS — can complete its handshake asynchronously and fire `ontrack` late, well after the card moved on. That unconditionally overwrote the video element back to WebRTC, hijacking it out from under an already-working HLS session (hls.js confirmed successfully fetching and playing the LL-HLS-stripped manifest at that exact point) and re-triggering the same dead-transport stall loop forever. Fixed with the same identity guard already proven on the sibling handlers. A 3-agent adversarial review of the fix caught a second, related bug it would otherwise have made reliably reachable: the abandoned attempt's own cleanup used shared instance fields that could by then belong to a *newer* attempt, closing the new live connection out from under itself — fixed by having the abandoned attempt clean up only its own state.
...
- **Remote-connection stream flapping continued even after the LL-HLS and stale-`ontrack` fixes above.** An `_startLiveVideo()` re-entrancy race: the function has two long `await`s (the `camera/stream` WS call, the hls.js CDN load) with no guard against a *newer* call superseding it while parked there — a teardown+fresh-start racing an old attempt's pending await let the old one resume afterward and destroy/reattach the shared `hls.js` instance a newer, already-healthy attempt owned, producing a repeating snapshot-flash-then-resume cycle. Fixed with a generation token (`_startGen`), checked after each `await`, mirroring the existing `this._webrtcPc !== pc` staleness guard pattern.
- **The stream still flapped after the re-entrancy fix**, but the pattern changed: it now played fine for a while, then silently froze and never recovered on its own — HLS's own fatal-error handler never fired (a frozen-but-not-fatal manifest), so the only pre-existing escalation path (to a full backend session rewarm) was unreachable. Root-caused via a dedicated review (this stream had genuinely worked before on this exact remote link): the generic stall/freeze recovery path had no escalation to a backend rewarm at all. Added one, escalating after repeated stalls within a rolling time window. First implementation used a plain reset-on-frame counter, which a live retest immediately falsified: a flapping source plays a few seconds every cycle (firing the same "frame rendered" event the counter reset on), wiping it every single cycle before it could ever escalate — replaced with a time-windowed count that isn't reset by a brief render, only by the window aging out once the stream is genuinely stable again. 3 independent adversarial reviews of the full fix (escalation + the backend rewarm's own auto-resume) found and fixed 4 further real bugs: the rewarm's auto-resume flag could leak past an unavailable-camera or already-on observation and later silently un-gate an unrelated remote autoplay; deliberate quality-mode restarts were being counted toward the stall-escalation window; the rewarm's own 20s cooldown was being wiped by the same brief-render pattern that broke the first counter, defeating its backoff for exactly the case it exists to protect; and the rewarm's turn-on timer wasn't cancelled on disconnect, so it could fire on a detached card instance.
- **Root-caused the underlying reason remote streams flap on a Cloudflare-Tunneled connection with no VPN at all** — none of the above (all genuine, now-fixed bugs) were the actual root cause. Traced to an open, unresolved upstream limitation ([cloudflare/cloudflared#1449](https://github.com/cloudflare/cloudflared/issues/1449)): GET requests can buffer completely through cloudflared regardless of header/content-type tricks, while POST requests stream fine — HLS delivery is all-GET. Confirmed the existing `cf_unbuffer.py` mitigation is active and correctly configured, and confirmed the same camera streams with no issues over a VPN instead of the tunnel. See the README's "Known Limitations" section and GitHub [#67](https://github.com/mosandlt/Bosch-Smart-Home-Camera-Tool-HomeAssistant/issues/67) for the full writeup and recommendation.
