...
class BoschCameraCard extends HTMLElement {
  ...
  _armStartUnmute() {
    ...
    if (this._isSecondaryAudioInstance) return;
    try {
  ...
  _refreshAudioPill() {
    ...
    const label = !live ? this._t("audio_available_when_live") : this._isSecondaryAudioInstance ? this._t("multi_audio_muted") : audible ? this._t("audio_mute") : this._t("audio_unmute");
    ...
    if (staleBadge) {
      if (privacyOn && lastEventStr !== "—") {
        ...
        try {
          ...
          if (d && !isNaN(d)) {
            const sameDay = d.toDateString() === (new Date).toDateString();
            staleTs = sameDay ? d.toLocaleTimeString([], {
              hour: "2-digit",
              minute: "2-digit"
