...
- **A REMOTE-mode live stream's connection badge could get permanently stuck on "Verbinde" (connecting) even though the video kept playing fine underneath.** The card's `requestVideoFrameCallback` liveness heartbeat only re-armed itself while `video.srcObject` was set (WebRTC playback) — REMOTE sessions mostly fall back to HLS/MSE (`.src`/`currentSrc`, no `srcObject`), so the heartbeat fired once on the first HLS frame and then never rearmed, leaving its "last frame" timestamp stale. ~10s later the freeze watchdog treated that staleness as a real freeze and permanently flipped the badge to "connecting", with no way to clear since no more heartbeat ticks would ever arrive to prove the stream was actually still alive. Fixed by also rearming on `video.currentSrc`/`video.getAttribute("src")`, matching an existing identical source-presence check already used elsewhere in the same file. Display-only bug — playback itself was never affected. 3-agent adversarial bug-hunt found no issues with the fix; confirmed `requestVideoFrameCallback` fires identically for HLS/MSE `<video src>` playback as for WebRTC, and that every other consumer of the same heartbeat timestamp is either WebRTC-gated or unaffected.
