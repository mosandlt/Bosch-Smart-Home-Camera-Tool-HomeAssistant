...
- **The RCP proxy host Bosch's cloud PUT /connection response hands back is now validated against the Bosch domain allowlist before being used to build any request URL**, at all three call sites that consume it (including the snap.jpg 404-retry path) — an unvalidated host there was a server-side request forgery (SSRF) path. The event-snapshot fetch also now disables HTTP redirects (`allow_redirects=False`): a validated `imageUrl` could otherwise still redirect to an arbitrary internal host, since `aiohttp` follows redirects by default and the existing allowlist check only validated the URL itself.
