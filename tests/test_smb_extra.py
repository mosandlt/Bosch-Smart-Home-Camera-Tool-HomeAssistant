...
# ── async_smb_disk_alert routing ─────────────────────────────────────────


class TestSmbDiskAlertRouting:
    """`async_smb_disk_alert` tries each comma-separated service; falls
    back to `persistent_notification.create` if none succeeded. Pinned
    so a typo ('notify.thomass') still results in a visible warning."""

    @pytest.mark.asyncio
    async def test_falls_back_to_persistent_notification_when_no_service(self):
        from custom_components.bosch_shc_camera.smb import async_smb_disk_alert
        # No service registered → has_service returns False → fall through
        hass = SimpleNamespace(
            services=SimpleNamespace(
                has_service=MagicMock(return_value=False),
                async_call=MagicMock(),
            )
        )
        # async_call is sync MagicMock — wrap return with awaitable
        async def _async_call(*args, **kwargs):
            return None
        hass.services.async_call = _async_call
        coord = SimpleNamespace(hass=hass)
        await async_smb_disk_alert(coord, "Disk full!", "notify.nonexistent")
        # Just verify no exception (assertion-light — function is mostly side-effect)

    @pytest.mark.asyncio
    async def test_calls_each_comma_separated_service(self):
        """Multiple services in one option string must each be called."""
        from custom_components.bosch_shc_camera.smb import async_smb_disk_alert
        called = []

        async def _async_call(domain, service, data, **_):
            called.append(f"{domain}.{service}")
        ...
        coord = SimpleNamespace(hass=hass)
        await async_smb_disk_alert(coord, "msg", "notify.a, notify.b , notify.c")
        assert called == ["notify.a", "notify.b", "notify.c"]

    @pytest.mark.asyncio
    async def test_empty_service_string_falls_back(self):
        """Empty string → no real services to try → persistent_notification
        fallback fires."""
        from custom_components.bosch_shc_camera.smb import async_smb_disk_alert
        called = []
        ...
        coord = SimpleNamespace(hass=hass)
        await async_smb_disk_alert(coord, "msg", "")
        assert called == ["persistent_notification.create"]
