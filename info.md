...
> **Quality Scale: Platinum** (v12.0.1, current release: v12.0.2) — `strict-typing` (mypy --strict green across all 24 source files, 0 errors), `async-dependency` (all `requests` imports removed; HTTP via aiohttp + new `auth_utils.async_digest_request`), runtime data on the config entry, raised + translatable service-action exceptions, downloadable diagnostics with secret redaction, repair issues for token-expired / Bosch-outage states, in-place reconfigure flow, automatic stale-device cleanup, full icon translations, pytest config-flow coverage. See `quality_scale.yaml` for the rule-by-rule status.
